...
async def get_import_status(
    ...
    try:
        ...
        raise BusinessError(
            code="IMPORT_STATUS_FAILED",
            message=f"获取导入状态失败: {str(e)}"
        )
...
async def get_data_stats(
    ...
    """获取数据统计"""
    try:
        stats = await etl_service.get_data_stats(
            tenant_id=current_user["tenant_id"]
        )
