...
async def train_model_from_upload(
    ...
    try:
        ...
        if not upload_result.success:
            raise HTTPException(
                status_code=400, detail=f"文件处理失败: {upload_result.message}"
            )
        ...
        config = ModelConfig(
            model_type=(
                "classification" if algorithm.endswith("_classifier") else "regression"
            ),
            algorithm=algorithm,
...
async def get_models(limit: int = 50, current_user: User = Depends(get_current_user)):
    """
    获取已训练的模型列表
...
async def predict_with_model(
    ...
    try:
        ...
        if missing_features:
            raise HTTPException(status_code=400, detail=f"缺少特征: {missing_features}")
...
async def batch_predict_with_model(
    ...
    try:
        logger.info(
            f"用户 {current_user.username} 使用模型 {model_id} 进行批量预测: {len(features_list)} 条"
        )
...
async def get_supported_algorithms():
    ...
    try:
        ...
        for (
            algo_name,
            algo_class,
        ) in model_training_service.supported_algorithms.items():
            algorithms[algo_name] = {
                "name": algo_name,
                "class": algo_class.__name__,
                "type": (
                    "classification"
                    if algo_name.endswith("_classifier")
                    else "regression"
                ),
                "default_parameters": model_training_service.default_parameters.get(
                    algo_name, {}
                ),
                "description": get_algorithm_description(algo_name),
            }

        return {"supported_algorithms": algorithms, "total_count": len(algorithms)}
...
async def get_training_stats(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        recent_models = [
            m for m in models if datetime.fromisoformat(m["created_at"]) > week_ago
        ]
        ...
        return {
            ...
            "supported_algorithms": list(
                model_training_service.supported_algorithms.keys()
            ),
        }
