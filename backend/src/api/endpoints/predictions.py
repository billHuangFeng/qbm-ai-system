...
class PredictionRequest(BaseModel):
    ...
    prediction_type: str = Field(
        "single", description="预测类型: single, batch, timeseries"
    )
    ...
    try:
        ...
        if request.apply_memory:
            try:
                ...
                memory_search_result = await memory_service.retrieve_relevant_memories(
                    ...
                    existing_memories=await get_existing_memories(
                        current_user["tenant_id"], db_service
                    ),
                    min_confidence=0.7,
                    min_relevance=0.6,
                )
                ...
                if memory_search_result:
                    # 应用记忆到预测
                    adjusted_prediction = (
                        await memory_service.apply_memory_to_prediction(
        ...
        if request.include_confidence:
            confidence_score = await calculate_prediction_confidence(
                model_info, request.input_data, prediction_result, db_service
            )
...
async def get_model_accuracy(
    ...
    try:
        ...
        if accuracy_stats["total_predictions"] > 0:
            accuracy_rate = (
                accuracy_stats["accurate_predictions"]
...
# 辅助函数
async def get_existing_memories(
    tenant_id: str, db_service: DatabaseService
) -> List[Dict[str, Any]]:
    """获取现有记忆"""
