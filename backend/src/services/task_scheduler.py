...
class TaskScheduler:
    ...
    def _calculate_next_run(
        self, schedule_type: ScheduleType, schedule_config: Dict[str, Any]
    ) -> Optional[datetime]:
        """计算下次执行时间"""
        try:
            current_time = datetime.now()
...
async def data_quality_check_task(dataset_id: str, dataset_name: str):
    ...
    try:
        ...
        return {
            "success": True,
            "message": f"Data quality check completed for {dataset_name}",
        }
...
async def report_generation_task(report_type: str, parameters: Dict[str, Any]):
    ...
    try:
        ...
        return {
            "success": True,
            "message": f"Report generation completed for {report_type}",
        }
