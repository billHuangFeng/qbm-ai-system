...
class TestLagAnalysis:
    ...
    def test_extract_key_lags(self):
        ...
        self.lag_analysis.lag_effects = {
            ...
            "lag_regression": {
                "feature1": {"best_lag": 1, "r2": 0.6, "coefficient": 0.5}
            },
            "granger_causality": {
                "feature1": {
                    "causality": True,
    ...
    def test_clear_lag_data(self):
        """测试明显滞后数据处理"""
        # 创建有明显滞后的数据
        clear_lag_X = pd.DataFrame({"feature1": np.random.normal(0, 1, 100)})
        clear_lag_y = pd.Series(
            clear_lag_X["feature1"].shift(2).fillna(0) + np.random.normal(0, 0.1, 100)
        )
    ...
    def test_negative_lag_handling(self):
        """测试负滞后处理"""
        # 创建有负滞后的数据（未来值影响当前值）
        negative_lag_X = pd.DataFrame({"feature1": np.random.normal(0, 1, 100)})
