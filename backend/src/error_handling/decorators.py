...
from .enhanced import (
    ...
    BMOSDatabaseError,
    BMOSBusinessError,
    BMOSAuthenticationError,
    BMOSAuthorizationError,
    BMOSExternalServiceError,
    BMOSConfigurationError,
    BMOSResourceError,
)
        ...
        async def async_wrapper(*args, **kwargs):
            try:
                ...
                error = BMOSValidationError(
                    message=f"输入验证失败: {str(e)}", details={"field": "unknown"}
                )
                if log_errors:
                ...
                error = BMOSResourceError(
                    message=f"缺少必要参数: {str(e)}", details={"missing_key": str(e)}
                )
                if log_errors:
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            try:
                ...
                if log_errors:
                    logger.warning(
                        f"Authentication error in {func.__name__}: {e.message}"
                    )
                from fastapi import HTTPException

                raise HTTPException(
                    status_code=401, detail={"code": e.code, "message": e.message}
                )
            except BMOSAuthorizationError as e:
                if log_errors:
                    logger.warning(
                        f"Authorization error in {func.__name__}: {e.message}"
                    )
                from fastapi import HTTPException
