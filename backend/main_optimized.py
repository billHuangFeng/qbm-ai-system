...
from src.error_handling.unified import (
    BMOSError, BusinessError, http_exception_handler,
    validation_exception_handler, generic_exception_handler, business_error_handler
)
# 导入日志配置
...
cache_service_instance = RedisCache(settings.redis.redis_url, settings.redis.redis_password)
...
async def lifespan(app: FastAPI):
    """应用生命周期管理"""
    logger.info(f"启动 {settings.application.app_name} v{settings.application.app_version}...")
...
async def root():
    ...
    """
    return {"message": "BMOS AI System API", "version": settings.application.app_version}
