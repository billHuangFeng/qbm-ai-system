...
class MasterDataMatcher(BaseService):
        ...
        for branch_id in self.branch_identifiers:
            if branch_id in full_name:
                ...
                if len(parts) >= 2:
                    ...
                    branch_name = (
                        branch_id + parts[1].strip() if parts[1].strip() else branch_id
                    )
    ...
    def calculate_name_similarity(
        ...
        if (
            not info1["is_branch"]
            and not info1["is_office"]
            and not info2["is_branch"]
            and not info2["is_office"]
        ):
        ...
        # 比较总公司名称是否一致
        headquarter1 = (
            info1["headquarter_name"]
            if info1["is_branch"] or info1["is_office"]
            else info1["full_name"]
        )
        headquarter2 = (
            info2["headquarter_name"]
            if info2["is_branch"] or info2["is_office"]
            else info2["full_name"]
        )
        ...
        if std_headquarter1 == std_headquarter2:
            ...
            # 一个是总公司，一个是分公司/办事处
                ...
                (info1["is_branch"] or info1["is_office"])
                and not info2["is_branch"]
                and not info2["is_office"]
            ):
                ...
                return (
                    0.60,
                    match_details,
                )  # 低置信度：应该匹配到分公司的记录，而不是总公司的

            elif (
                (info2["is_branch"] or info2["is_office"])
                and not info1["is_branch"]
                and not info1["is_office"]
            ):
                ...
                return (
                    0.60,
                    match_details,
                )  # 低置信度：应该匹配到总公司的记录，而不是分公司的

        # 场景3：总公司名称不一致，即使有分公司/办事处标识，也需要比较总公司部分
        headquarter_sim = self._calculate_name_similarity_core(
            std_headquarter1, std_headquarter2
        )
    ...
    async def fetch_master_data(
        self, data_type: str, master_data_table: str, tenant_id: str
    ) -> pd.DataFrame:
        """
        从数据库获取主数据
    ...
    def match_single_record(
        ...
        for _, master_row in master_data.iterrows():
            ...
            if data_type == "product":
                master_specification = (
                    master_row.get("specification")
                    or master_row.get("spec")
                    or master_row.get("model")
                    or master_row.get("model_number")
                    or master_row.get("规格型号")
                    or master_row.get("型号规格")
                )

            # 计算名称相似度（考虑总公司和分公司/办事处）
            name_sim, name_match_details = self.calculate_name_similarity(
                record_name, master_name
            )

            # 如果有别名，也计算相似度
            if master_alias:
                alias_sim, alias_match_details = self.calculate_name_similarity(
                    record_name, master_alias
                )
            ...
            # 产品匹配：无规格型号时，仅依赖名称匹配
                ...
                data_type == "product"
                and not record_specification
                and not master_specification
            ):
        ...
        # 添加名称匹配类型信息
        name_match_type = (
            best_match.get("name_match_type", "unknown") if best_match else "unknown"
        )
        name_match_details = (
            best_match.get("name_match_details", {}) if best_match else {}
        )
    ...
    def _generate_match_reason(
        self, match: Optional[Dict[str, Any]], record_name: str, record_code: str
    ) -> str:
        """生成匹配原因描述"""
        if not match:
            return "未找到匹配的主数据"
        ...
        if code_fully_matched:
            ...
            reasons.append(
                f"代码存在细微差异（约{edit_distance}个字符，相似度{code_sim*100:.0f}%）"
            )
        elif code_sim > 0:
        ...
        if name_match_type == "same_headquarter_same_branch":
            ...
            reasons.append(
                "【低置信度】记录是总公司，主数据是分公司/办事处，应该匹配到总公司的记录"
            )
        elif name_match_type == "branch_to_headquarter":
            reasons.append(
                "【低置信度】记录是分公司/办事处，主数据是总公司，应该匹配到分公司的记录"
            )
    ...
    async def match_master_data(
        ...
        try:
            ...
            master_data = await self.fetch_master_data(
                data_type, master_data_table, tenant_id
            )
