...
class BMOSCorePerformanceTester:
    ...
    def test_model_training_performance(self):
        ...
        for size in test_sizes:
            for algorithm in algorithms:
                ...
                X_prepared, y_prepared = self.model_training_service.prepare_data(df, config)
                data_preparation_time = time.time() - start_time
                
                # 训练模型
                start_time = time.time()
                model, metrics = self.model_training_service.train_model(X_prepared, y_prepared, config)
                training_time = time.time() - start_time
                
                # 评估模型
                start_time = time.time()
                evaluation = self.model_training_service.evaluate_model(model, X_prepared, y_prepared, config)
                evaluation_time = time.time() - start_time
                
                # 预测测试
                start_time = time.time()
                prediction = self.model_training_service.predict(model, X_prepared.iloc[:10])
    ...
    async def test_enterprise_memory_performance(self):
        ...
        for size in test_sizes:
            ...
            # 测试模式提取性能
            start_time = time.time()
            patterns = await self.memory_service.extract_patterns_from_data(test_data, "target")
            ...
            # 测试推荐生成性能
            start_time = time.time()
            recommendations = await self.memory_service.generate_recommendations(insights)
