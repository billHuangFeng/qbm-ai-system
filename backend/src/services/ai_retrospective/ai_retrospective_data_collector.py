...
class AIRetrospectiveDataCollector:
    ...
    async def collect_decision_outcomes(
        ...
        """
        try:
            logger.info(f"开始收集决策执行结果: decision_id={decision_id}, session_id={session_id}")
    ...
    async def monitor_metric_changes(
        ...
        """
        try:
            logger.info(f"开始监控指标变化: metric_id={metric_id}, session_id={session_id}")
            ...
            historical_data = await self._get_metric_history(metric_id, start_time, end_time)
    ...
    async def detect_anomalies(
        ...
        """
        try:
            logger.info(f"开始异常检测: data_count={len(data)}, anomaly_type={anomaly_type}")
            
            if not data or len(data) < 3:
                return {
                    "success": False,
                    "error": "数据量不足，无法进行异常检测"
                }
        ...
        try:
            ...
            if self.memory_service:
                await self._record_to_memory(
                    ...
                    content=data_content,
                    tags=["retrospective", "feedback", validated_feedback.get("type", "general")]
                )
    ...
    async def _get_decision_baseline(self, decision_id: str) -> Optional[Dict[str, Any]]:
        """获取决策基线"""
        try:
    ...
    def _calculate_deviation(
        ...
        for key, actual_value in outcome_data.items():
            if key in baseline_values:
                ...
                if isinstance(actual_value, (int, float)) and isinstance(baseline_value, (int, float)):
                    if baseline_value != 0:
        ...
        return {
            ...
            "overall_deviation": np.mean([abs(d["deviation"]) for d in deviations.values()]) if deviations else 0
        }
    ...
    async def _detect_threshold_anomalies(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        """使用阈值方法检测异常"""
        anomalies = []
