...
class DataQualityCheckerRefactored:
    ...
    async def _execute_quality_checks(
        self, df: pd.DataFrame, custom_rules: Optional[List] = None
    ) -> Dict[str, Any]:
        """执行质量检查"""
        quality_results = {}
    ...
    async def _generate_quality_report(
        ...
        # 计算总体分数
        overall_score = (
            sum(metrics_scores.values()) / len(metrics_scores)
            if metrics_scores
            else 0.0
        )
    ...
    async def _check_custom_rules(
        self, df: pd.DataFrame, custom_rules: List
    ) -> Dict[str, Any]:
        """检查自定义规则"""
        custom_score = 1.0
        issues = []

        return {"metric": "custom", "score": custom_score, "issues": issues}
    ...
    def _generate_recommendations(
        ...
        if critical_issues:
            recommendations.append(
                f"发现 {len(critical_issues)} 个严重质量问题，需要立即处理"
            )
