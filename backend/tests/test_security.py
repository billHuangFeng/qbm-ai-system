...
class TestSecurityControls:
    ...
    async def test_cross_tenant_data_access(self, other_tenant_user):
        ...
        with patch('backend.src.api.endpoints.optimization.get_db') as mock_db:
            # 模拟数据库查询结果（其他租户的数据）
            mock_row = (1, "uuid1", "tenant_123", "performance", "其他租户建议", "描述", 
                       "high", 8.5, "medium", 1.5, "pending", None, None)
    ...
    async def test_sensitive_data_logging(self, sensitive_user):
        ...
        with patch('backend.src.api.endpoints.monitoring.get_db') as mock_db:
            ...
            with patch('backend.src.api.endpoints.monitoring.require_permission') as mock_auth:
                ...
                with patch('backend.src.api.endpoints.monitoring.logger') as mock_logger:
...
class TestRateLimiting:
    ...
    async def test_rate_limiting_protection(self, rate_limit_user):
        ...
        # 统计被限制的请求数量
        rate_limited_count = sum(1 for result in results if isinstance(result, Exception))
