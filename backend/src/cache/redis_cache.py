...
class RedisCache:
    ...
    async def connect(self):
        ...
        try:
            ...
            self.is_connected = False
            raise BusinessError(
                code="REDIS_CONNECTION_FAILED", message=f"Redis连接失败: {str(e)}"
            )
    ...
    async def set(
        ...
        if not self.is_connected:
            raise BusinessError(code="REDIS_NOT_CONNECTED", message="Redis未连接")
        ...
        try:
            ...
            raise BusinessError(
                code="CACHE_SET_FAILED", message=f"设置缓存失败: {str(e)}"
            )
