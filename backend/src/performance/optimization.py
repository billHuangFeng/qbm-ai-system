...
class PerformanceOptimizedService:
    ...
    async def paginated_query(
        ...
        try:
            ...
            count_result = await self.db_service.execute_query(
                count_query, where_params or [], fetch_one=True
            )
    ...
    async def batch_load_with_relations(
        ...
        for relation_name, relation_config in relation_tables.items():
            ...
            try:
                relation_results = await self.db_service.execute_query(
                    relation_query, main_ids, fetch_all=True
                )
        ...
        for main_item in main_data.data:
            ...
            for relation_name in relations_data:
                item[relation_name] = relations_data[relation_name].get(
                    main_item["id"], []
                )
            result.append(item)
    ...
    async def get_aggregated_stats(
        ...
        try:
            results = await self.db_service.execute_query(
                query, where_params or [], fetch_all=True
            )
            return results or []
    ...
    async def execute_in_batches(
        self, operation: str, data: List[Any], batch_size: int = 1000
    ) -> int:
        """批量执行操作"""
