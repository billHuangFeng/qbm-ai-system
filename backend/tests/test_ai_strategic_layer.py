...
from src.services.ai_strategic_layer.ai_decision_requirements_service import (
...
class TestAINorthStarService:
    ...
    def north_star_service(self, mock_db_service, mock_memory_service):
        ...
        return AINorthStarService(
            db_service=mock_db_service, memory_service=mock_memory_service
        )
    ...
    async def test_calculate_ai_weight(self, north_star_service, mock_db_service):
        ...
        result = await north_star_service._calculate_ai_weight(
            metric_id=TEST_METRIC_ID, strategic_objective_id=TEST_STRATEGIC_OBJECTIVE_ID
        )
    ...
    @pytest.mark.asyncio
    async def test_calculate_metric_health_score(
        self, north_star_service, mock_db_service
    ):
        """测试指标健康度评分"""
        # 模拟指标数据
...
class TestAIDecisionRequirementsService:
    ...
    async def test_find_similar_requirements(
        self, requirements_service, mock_memory_service, mock_db_service
    ):
        """测试查找相似需求"""
        # 模拟企业记忆系统
        mock_pattern = MagicMock()
        ...
        result = await requirements_service._find_similar_requirements(
            requirement_description="需要增加预算", requirement_type="strategic"
        )
...
class TestAIStrategicLayerAPI:
    ...
    async def test_recommend_metrics_endpoint(self, client, mock_current_user):
        """测试指标推荐端点"""
        with (
            patch("src.api.endpoints.ai_strategic_layer.get_current_user") as mock_auth,
            patch(
                "src.api.endpoints.ai_strategic_layer.get_north_star_service"
            ) as mock_service,
        ):
            ...
            mock_north_star.recommend_metrics = AsyncMock(
                return_value=[
                    {
                        "metric_name": "用户增长率",
                        "metric_type": "growth",
            ...
            response = client.post(
                ...
                json={"strategic_objective_id": TEST_STRATEGIC_OBJECTIVE_ID},
    ...
    async def test_get_metric_health_endpoint(self, client, mock_current_user):
        ...
        with (
            ...
            mock_north_star = AsyncMock()
            mock_north_star.calculate_metric_health_score = AsyncMock(
                return_value={
                    "health_score": 0.75,
                    "status": "good",
                    "factors": {
...
class TestServiceIntegration:
    ...
    async def test_end_to_end_okr_workflow(self):
        ...
        # 创建服务实例
        okr_service = AIOKRService(db_service=mock_db, memory_service=mock_memory)
