...
class AIRetrospectiveAnalyzer:
    ...
    async def analyze_root_causes(
        ...
        """
        try:
            logger.info(f"开始根因分析: session_id={session_id}, depth={analysis_depth}")
    ...
    async def identify_patterns(
        ...
        """
        try:
            logger.info(f"开始模式识别: session_id={session_id}, pattern_type={pattern_type}")
            
            if not historical_data or len(historical_data) < 5:
                return {
                    "success": False,
                    "error": "历史数据不足，无法进行模式识别"
                }
    ...
    async def extract_success_factors(
        ...
        """
        try:
            logger.info(f"开始提取成功因素: session_id={session_id}, cases={len(success_cases)}")
            ...
            # 2. 使用协同分析识别成功因素
            success_factors = await self._analyze_success_factors(success_cases, common_features)
            
            # 3. 评估因素重要性
            factor_scores = self._score_success_factors(success_factors, success_cases)
            
            # 4. 识别最佳实践
            best_practices = await self._identify_best_practices(success_factors, success_cases)
    ...
    async def analyze_failure_reasons(
        ...
        """
        try:
            logger.info(f"开始分析失败原因: session_id={session_id}, cases={len(failure_cases)}")
            ...
            # 4. 识别风险因素
            risk_factors = await self._identify_risk_factors(failure_features, failure_patterns)
        ...
        try:
            if self.memory_service:
                ...
                try:
                    ...
                    patterns = await self.memory_service.search_similar_patterns(query, limit=10)
    ...
    async def _identify_root_causes(
        ...
        if analysis_depth in ["comprehensive", "deep"]:
            if features.get("common_issue_types"):
                most_common = max(
                    features["common_issue_types"].items(),
                    key=lambda x: x[1]
                )
                root_causes.append({
    ...
    def _calculate_pattern_confidence(self, patterns: Dict[str, Dict[str, Any]]) -> float:
        """计算模式识别置信度"""
        if not patterns:
            return 0.0
    ...
    def _extract_common_features(self, success_cases: List[Dict[str, Any]]) -> Dict[str, Any]:
        """提取共同特征"""
        common_features = {}
        ...
        for key, value in common_features.items():
            if key not in ["id", "timestamp", "created_at"]:
                factors.append({
                    ...
                    "importance": "high" if isinstance(value, (int, float)) else "medium"
                })
        ...
        for factor in success_factors:
            if factor.get("frequency", 0) >= len(success_cases) * 0.8:
                practices.append({
                    ...
                    "reason": "在所有成功案例中都出现",
                    "confidence": factor.get("frequency", 0) / len(success_cases) if success_cases else 0.0
                })
    ...
    def _extract_failure_features(self, failure_cases: List[Dict[str, Any]]) -> Dict[str, Any]:
        """提取失败特征"""
        features = {}
