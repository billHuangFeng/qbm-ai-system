...
class KnowledgeSearchService:
        ...
        try:
            ...
            if self.semantic_model:
                ...
                return await self.keyword_search(
                    query, tenant_id, domain_category, problem_type, limit
                )
            ...
            for candidate in candidates:
                ...
                # 计算余弦相似度
                similarity = float(
                    np.dot(query_embedding, knowledge_embedding)
                    / (
                        np.linalg.norm(query_embedding)
                        * np.linalg.norm(knowledge_embedding)
                    )
                )
    ...
    async def relevance_ranking(
        ...
        try:
            ...
            # 按相关性排序
            knowledge_list.sort(
                key=lambda x: x.get("calculated_relevance", 0), reverse=True
            )
    ...
    async def recommend_knowledge(
        ...
        try:
            ...
            # 过滤：优先推荐已验证的知识
            verified_results = [
                r for r in ranked_results if r.get("verification_status") == "verified"
            ]
            other_results = [
                r for r in ranked_results if r.get("verification_status") != "verified"
            ]
            ...
            final_results = (
                verified_results[:limit]
                + other_results[: max(0, limit - len(verified_results))]
            )
    ...
    def _calculate_context_relevance(
        ...
        # 知识类型匹配（如果有）
        if context.get("knowledge_type") and knowledge.get(
            "knowledge_type"
        ) == context.get("knowledge_type"):
            relevance += 0.2
