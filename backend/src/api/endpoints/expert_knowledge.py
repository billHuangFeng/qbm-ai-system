...
from ...services.expert_knowledge import ExpertKnowledgeService, DocumentProcessingService, KnowledgeIntegrationService, KnowledgeSearchService
...
class ApplyKnowledgeRequest(BaseModel):
    ...
    application_type: str = Field(..., description="应用方式: reference/reasoning/validation")
...
class VerifyKnowledgeRequest(BaseModel):
    """验证知识请求"""
    verification_status: str = Field(..., description="验证状态: pending/verified/rejected")
...
async def import_knowledge(
    ...
    try:
        ...
        # 提取文本
        extract_result = await doc_service.extract_text_from_file(str(file_path), file_type)
...
async def get_knowledge(
    ...
    """获取知识详情"""
    try:
        knowledge = await service.get_knowledge_by_id(knowledge_id, current_user["tenant_id"])
...
async def get_domain_categories():
    ...
    return {
        "success": True,
        "categories": [
            "business_model", "cost_optimization", "resource_allocation",
            "capability_enhancement", "market_strategy", "product_design",
            "risk_management", "performance_measurement"
        ]
    }
...
async def get_problem_types():
    ...
    return {
        "success": True,
        "problem_types": [
            "decision_problem", "optimization_problem", "risk_problem",
            "innovation_problem", "retrospective_problem"
        ]
    }
...
async def get_knowledge_types():
    ...
    return {
        "success": True,
        "knowledge_types": [
            "theory", "methodology", "case_study",
            "tool_template", "best_practice", "warning"
        ]
    }
...
async def generate_reasoning_chain(
    ...
    current_user: Dict[str, Any] = Depends(get_current_user_mock),
    integration_service: KnowledgeIntegrationService = Depends(get_knowledge_integration_service)
):
