...
class TestDataPreprocessingService:
    ...
    def test_detect_outliers_zscore(self):
        ...
        outliers = self.service.detect_outliers_zscore(
            test_data, "rd_asset", threshold=3
        )
    ...
    def test_handle_missing_values_mean(self):
        ...
        filled_data = self.service.handle_missing_values(
            test_data, "rd_asset", method="mean"
        )
    ...
    def test_handle_missing_values_median(self):
        ...
        filled_data = self.service.handle_missing_values(
            test_data, "rd_asset", method="median"
        )
    ...
    def test_handle_missing_values_forward_fill(self):
        ...
        filled_data = self.service.handle_missing_values(
            test_data, "rd_asset", method="forward_fill"
        )
    ...
    def test_standardize_data(self):
        ...
        standardized_data = self.service.standardize_data(
            test_data, ["rd_asset", "design_asset"]
        )
    ...
    def test_outlier_treatment_options(self):
        ...
        # 测试删除异常值
        cleaned_data = self.service.treat_outliers(
            test_data, "rd_asset", method="remove"
        )
        assert len(cleaned_data) < len(test_data)

        # 测试替换异常值
        replaced_data = self.service.treat_outliers(
            test_data, "rd_asset", method="replace"
        )
    ...
    def test_data_transformation(self):
        ...
        # 测试对数转换
        transformed_data = self.service.transform_data(
            test_data, "rd_asset", method="log"
        )
        assert "rd_asset_log" in transformed_data.columns

        # 测试平方根转换
        transformed_data = self.service.transform_data(
            test_data, "design_asset", method="sqrt"
        )
    ...
    def test_data_imputation_strategies(self):
        ...
        # 测试KNN插补
        knn_imputed = self.service.impute_missing_values(
            test_data, "rd_asset", method="knn"
        )
        assert not knn_imputed["rd_asset"].isna().any()

        # 测试迭代插补
        iterative_imputed = self.service.impute_missing_values(
            test_data, "rd_asset", method="iterative"
        )
