...
from ...services.ai_strategic_layer.ai_strategic_objectives_service import (
...
from ...services.ai_strategic_layer.ai_decision_requirements_service import (
...
def get_strategic_objectives_service(
    ...
    """获取战略目标服务"""
    return AIStrategicObjectivesService(
...
class SynergyAnalysisRequest(BaseModel):
    ...
    related_objective_ids: Optional[List[str]] = Field(
        None, description="相关目标ID列表"
    )
...
class CreateRequirementRequest(BaseModel):
    ...
    requirement_type: str = Field(
        ..., description="需求类型: strategic, tactical, operational, emergency"
    )
    ...
    try:
        ...
        if len(decisions) < 2:
            return ConflictPredictionResponse(
                conflicts=[], conflict_count=0, overall_risk_level="low"
            )
        ...
        overall_risk = (
            "high" if len(conflicts) > 3 else "medium" if len(conflicts) > 0 else "low"
        )
...
async def get_requirement_priority(
    ...
    try:
        ...
        return {
            ...
            "priority_level": (
                priority_analysis.get("priority_level", 5) if priority_analysis else 5
            ),
...
async def get_okrs_by_objective(
    ...
    try:
        okrs = await service.get_okrs_by_objective(
            strategic_objective_id=strategic_objective_id, status=status
        )
