...
class AIOKRService:
    ...
    async def _predict_achievement_probability(self, okr_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 3. 尝试使用历史数据训练模型
            historical_data = await self._get_historical_okr_data(
                okr.get("strategic_objective_id")
            )
            ...
            if historical_data and len(historical_data) >= 10:
                try:
                    ...
                    return {
                        ...
                        "factors": self._analyze_achievement_factors(
                            feature_data, okr, krs
                        ),
                    }
                except Exception as e:
    ...
    async def _prepare_prediction_features(
        self, okr: Dict[str, Any], krs: List[Dict[str, Any]]
    ) -> Dict[str, float]:
        """准备预测特征"""
        try:
            features = {}
            ...
            if krs:
                ...
                features["kr_completion_rate"] = (
                    completed_krs / len(krs) if len(krs) > 0 else 0
                )

                # KR风险数量
                at_risk_krs = sum(1 for kr in krs if kr.get("status") == "at_risk")
                features["kr_risk_ratio"] = (
                    at_risk_krs / len(krs) if len(krs) > 0 else 0
                )
            ...
            if self.db_service:
                ...
                if strategic_obj_id:
                    ...
                    if historical_okrs:
                        success_count = sum(
                            1 for h in historical_okrs if h.get("status") == "completed"
                        )
                        features["historical_success_rate"] = success_count / len(
                            historical_okrs
                        )
                    else:
    ...
    async def _get_historical_okr_data(
        self, strategic_objective_id: Optional[str]
    ...
    async def _rule_based_prediction(
        ...
        try:
            ...
            return {"probability": 0.5, "method": "rule_based_error", "confidence": 0.3}
    ...
    async def _recommend_best_practices(
        self, objective_statement: str, strategic_objective_id: str
    ) -> List[Dict[str, Any]]:
        """AI推荐最佳实践（使用企业记忆系统）"""
        try:
            recommendations = []
    ...
    async def _identify_risk_factors(
        self, okr_id: str, achievement_prediction: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """识别风险因素"""
        try:
            risk_factors = []
    ...
    async def _update_okr_progress(self, okr_id: str) -> None:
        ...
        try:
            ...
            weighted_progress = sum(
                kr.get("current_progress", 0) * kr.get("weight", 1.0) for kr in krs
            )
            ...
            if current_value is not None:
                ...
                if target_value and target_value != 0:
                    calculated_progress = min(
                        100.0, max(0.0, (current_value / target_value) * 100)
                    )
    ...
    async def get_okrs_by_objective(
        self, strategic_objective_id: str, status: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """获取指定战略目标下的所有OKR"""
        try:
            if not self.db_service:
