...
# 创建简化的FastAPI应用
app = FastAPI(title="BMOS API Test", description="BMOS系统API测试", version="1.0.0")
...
async def test_endpoint():
    ...
    return {
        ...
        "data": {"test_id": "test_001", "test_time": datetime.now().isoformat()},
    }
        ...
        async with httpx.AsyncClient() as client:
            ...
            try:
                ...
                response = await client.post(
                    f"{self.base_url}/test/post", json=test_data
                )
                print(f"   状态码: {response.status_code}")
                print(f"   响应: {response.json()}")
            except Exception as e:
