...
class DocumentFormatDetector(BaseService):
    ...
    def _detect_repeated_header(
        self, data: pd.DataFrame
    ) -> Tuple[float, Dict[str, Any]]:
        """检测重复单据头格式（格式1）"""
        # 检查是否有单据号字段
        doc_number_cols = self._find_columns(
            data, ["单据号", "document_number", "document_id", "单号"]
        )
    ...
    def _detect_separate_header_body(
        self, data: pd.DataFrame
    ) -> Tuple[float, Dict[str, Any]]:
        """检测单据头和明细分离格式（格式3）"""
    ...
    def _detect_pure_header(self, data: pd.DataFrame) -> Tuple[float, Dict[str, Any]]:
        ...
        for col in detail_cols:
            if (
                data[col].isna().all()
                or (data[col].astype(str).str.strip() == "").all()
            ):
