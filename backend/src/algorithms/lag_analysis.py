...
class LagAnalysis:
    ...
    def detect_lag_effects(
        ...
        try:
            ...
            # 1. 交叉相关分析
            cross_correlation = self._analyze_cross_correlation(
                X, y, max_lag, min_correlation
            )
    ...
    def _analyze_cross_correlation(
        self, X: pd.DataFrame, y: pd.Series, max_lag: int, min_correlation: float
    ) -> Dict[str, Any]:
        """分析交叉相关"""
        try:
            cross_corr_results = {}
                    ...
                    if len(feature_values) > lag:
                        ...
                        if len(lagged_feature) > 10:  # 确保有足够的样本
                            ...
                            if (
                                ...
                                and abs(correlation) >= min_correlation
                            ):
            ...
            logger.info(
                f"交叉相关分析完成，发现 {len(cross_corr_results)} 个特征的滞后效应"
            )
    ...
    def _analyze_lag_regression(
        self, X: pd.DataFrame, y: pd.Series, max_lag: int
    ) -> Dict[str, Any]:
        """分析滞后回归"""
        try:
            ...
            for feature in X.columns:
                ...
                if lag_performance:
                    # 找到最佳滞后
                    best_lag = max(
                        lag_performance.keys(), key=lambda k: lag_performance[k]["r2"]
                    )
            ...
            logger.info(
                f"滞后回归分析完成，发现 {len(lag_regression_results)} 个特征的最佳滞后"
            )
        ...
        try:
            ...
            for feature in X.columns:
                        ...
                        if len(lagged_feature) > 20:  # 确保有足够的样本
                            ...
                            X_unrestricted = np.column_stack(
                                [
                                    ...
                                    (
                                        corresponding_y[:-1]
                                        if len(corresponding_y) > 1
                                        else lagged_feature[:-1]
                                    ),
                                ]
                            )
                            y_unrestricted = (
                            ...
                            if len(X_unrestricted) > 10:
                                ...
                                # 约束模型（不包含滞后特征）
                                X_restricted = (
                                    corresponding_y[:-1].reshape(-1, 1)
                                    if len(corresponding_y) > 1
                                    else lagged_feature[:-1].reshape(-1, 1)
                                )
                                y_restricted = (
                                    corresponding_y[1:]
                                    if len(corresponding_y) > 1
                                ...
                                if len(X_restricted) > 10:
                                    ...
                                    # 简化的p值计算
                                    p_value = self._calculate_f_p_value(
                                        f_stat, 1, n - k
                                    )
                ...
                if causality_tests:
                    ...
                    significant_lags = {
                        lag: test
                        for lag, test in causality_tests.items()
                        if test["causality"]
                    }

                    if significant_lags:
                        best_lag = min(
                            significant_lags.keys(),
                            key=lambda k: significant_lags[k]["p_value"],
                        )
            ...
            logger.info(
                f"格兰杰因果性分析完成，发现 {len([r for r in granger_results.values() if r['causality']])} 个特征的因果性"
            )
    ...
    def _select_optimal_lags(
        ...
        try:
            ...
            logger.info(
                f"最优滞后选择完成，为 {len(optimal_lags)} 个特征选择了最优滞后"
            )
            return optimal_lags
    ...
    def _calculate_lag_significance(
        self, feature_values: np.ndarray, y_values: np.ndarray, lag: int
    ) -> float:
        """计算滞后显著性"""
        try:
            if lag == 0:
        ...
        try:
            ...
            if "cross_correlation" in lag_results and lag_results["cross_correlation"]:
                ...
                cross_corr_avg = np.mean(
                    [
                        ...
                        for data in lag_results["cross_correlation"].values()
                    ]
                )
                score += (
                    weights["cross_correlation"] * cross_corr_count * cross_corr_avg
                )
            ...
            if "granger_causality" in lag_results and lag_results["granger_causality"]:
                granger_count = len(
                    [
                        r
                        for r in lag_results["granger_causality"].values()
                        if r["causality"]
                    ]
                )
                score += (
                    weights["granger_causality"] * granger_count * 0.5
                )  # 因果性得分为0.5
    ...
    def create_lag_features(
        self, X: pd.DataFrame, lag_threshold: float = 0.1
    ) -> pd.DataFrame:
        """创建滞后特征"""
        try:
            X_enhanced = X.copy()
            ...
            if "optimal_lags" in self.lag_effects:
                for feature, optimal_lag in self.lag_effects["optimal_lags"].items():
                    if optimal_lag > 0:
                        ...
                        # 创建滞后交互特征
                        X_enhanced[f"{feature}_lag_interaction"] = (
                            X[feature] * lagged_values
                        )
            ...
            logger.info(
                f"滞后特征创建完成，新增 {len(X_enhanced.columns) - len(X.columns)} 个特征"
            )
            return X_enhanced
    ...
    def get_lag_insights(self) -> Dict[str, Any]:
        ...
        try:
            insights = {
                ...
                "lag_level": self._classify_lag_level(
                    self.lag_effects.get("overall_score", 0)
                ),
    ...
    def _extract_key_lags(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            key_lags.sort(
                key=lambda x: x.get(
                    "correlation", x.get("r2", x.get("f_statistic", 0))
                ),
                reverse=True,
            )
