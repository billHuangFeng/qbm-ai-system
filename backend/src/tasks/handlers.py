    ...
    def _register_handlers(self):
        ...
        asyncio.create_task(
            self.task_manager.register_handler(
                "default", "data_processing", self.process_data
            )
        )
        asyncio.create_task(
            self.task_manager.register_handler(
                "default", "model_training", self.train_model
            )
        )
        asyncio.create_task(
            self.task_manager.register_handler(
                "default", "prediction_batch", self.batch_predictions
            )
        )
        asyncio.create_task(
            self.task_manager.register_handler(
                "default", "memory_extraction", self.extract_memory
            )
        )
        asyncio.create_task(
            self.task_manager.register_handler(
                "default", "data_quality_check", self.check_data_quality
            )
        )
        asyncio.create_task(
            self.task_manager.register_handler(
                "default", "system_cleanup", self.system_cleanup
            )
        )

        # 注册调度器函数
        asyncio.create_task(
            self.scheduler_service.register_function(
                "daily_model_retrain", self.daily_model_retrain
            )
        )
        asyncio.create_task(
            self.scheduler_service.register_function(
                "hourly_data_sync", self.hourly_data_sync
            )
        )
        asyncio.create_task(
            self.scheduler_service.register_function(
