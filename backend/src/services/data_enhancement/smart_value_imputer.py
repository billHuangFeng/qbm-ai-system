...
class SmartValueImputer(BaseService):
    ...
    def select_imputation_strategy(
        self, field_config: Dict[str, Any], missing_ratio: float, field_type: str
    ) -> str:
        """
        自动选择补值策略
    ...
    def impute_with_knn(
        self, df: pd.DataFrame, columns: List[str], n_neighbors: int = 5
        ...
        try:
            # 只处理数值型列
            numeric_columns = (
                df[columns].select_dtypes(include=[np.number]).columns.tolist()
            )
    ...
    def impute_with_iterative(
        self, df: pd.DataFrame, columns: List[str], max_iter: int = 10
    ) -> Tuple[pd.DataFrame, List[Dict[str, Any]]]:
        """
        使用迭代补值（MICE方法）
        ...
        try:
            ...
            imputer = IterativeImputer(
                max_iter=max_iter, random_state=42, imputation_order="ascending"
            )
    ...
    def impute_with_random_forest(
        ...
        try:
            for col in columns:
                ...
                # 准备训练数据
                X_known = df.loc[
                    known_mask, df.columns.difference([col])
                ...
                X_missing = df.loc[
                    missing_mask, df.columns.difference([col])
                ].select_dtypes(include=[np.number])
                    ...
                    imputation_log.append(
                        {
                            ...
                            "original_value": None,
                            "imputed_value": (
                                str(predicted_values[idx])
                                if isinstance(predicted_values[idx], (str, object))
                                else float(predicted_values[idx])
                            ),
    ...
    async def impute_values(
        ...
        try:
            ...
            if risk_assessment["blocked_fields"] and skip_blocked_fields:
                logger.warning(
                    f"以下字段被阻止补值: {[f['field'] for f in risk_assessment['blocked_fields']]}"
                )
                # 从配置中移除被阻止的字段
            ...
            if strategy == "auto":
                ...
                for field_name, field_config in field_configs.items():
                    ...
                    # 检查是否允许补值
                    allow, reason = self.should_allow_imputation(
                    ...
                    selected_strategy = self.select_imputation_strategy(
                        field_config, missing_ratio, field_type
                    )
                    ...
                    if selected_strategy == "knn":
                        ...
                        df_imputed, log = self.impute_with_iterative(
                            df_imputed, [field_name]
                        )
                        all_imputation_log.extend(log)
                    elif selected_strategy == "random_forest":
                        df_imputed, log = self.impute_with_random_forest(
                            df_imputed, [field_name]
                        )
                        all_imputation_log.extend(log)
                    elif selected_strategy == "rule_based":
                        df_imputed, log = self.impute_with_rule(
                            df_imputed, {field_name: field_config}
                        )
                        all_imputation_log.extend(log)

            elif strategy == "knn":
                columns_with_missing = missing_info["columns_with_missing"]
                df_imputed, all_imputation_log = self.impute_with_knn(
                    df_imputed, columns_with_missing
                )

            elif strategy == "iterative":
                columns_with_missing = missing_info["columns_with_missing"]
                df_imputed, all_imputation_log = self.impute_with_iterative(
                    df_imputed, columns_with_missing
                )

            elif strategy == "random_forest":
                columns_with_missing = missing_info["columns_with_missing"]
                df_imputed, all_imputation_log = self.impute_with_random_forest(
                    df_imputed, columns_with_missing
                )

            elif strategy == "rule_based":
                df_imputed, all_imputation_log = self.impute_with_rule(
                    df_imputed, field_configs
                )
            ...
            imputation_rate = (
                imputed_count / missing_info["total_missing"]
                if missing_info["total_missing"] > 0
                else 0.0
            )
            ...
            statistics = {
                ...
                "fields_imputed": list(
                    set([log["field"] for log in all_imputation_log])
                ),
            ...
            for log in all_imputation_log:
                ...
                if field_name in field_configs:
                    ...
                    log["business_critical"] = field_config.get(
