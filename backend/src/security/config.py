...
class SecuritySettings(BaseSettings):
    ...
    def validate_cors_origins(cls, v):
        if "*" in v:
            logger.warning("CORS origins contains wildcard (*), this may be a security risk")
        return v
...
def validate_password_strength(password: str, settings: SecuritySettings) -> tuple[bool, list[str]]:
    """验证密码强度"""
    errors = []
...
def validate_file_upload(file_name: str, file_size: int, settings: SecuritySettings) -> tuple[bool, str]:
    """验证文件上传安全性"""
