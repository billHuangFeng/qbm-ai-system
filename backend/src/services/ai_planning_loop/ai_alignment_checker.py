...
class AIAlignmentChecker:
    ...
    async def check_decision_alignment(
        ...
        """
        try:
            logger.info(f"开始检查决策对齐: decision_id={decision_id}, check_type={check_type}")
            ...
            # 6. 确定对齐状态
            alignment_status = self._determine_alignment_status(alignment_score, check_results)
            
            # 7. 保存检查结果到数据库
            check_id = await self._save_alignment_check(
                decision_id, check_type, alignment_status, alignment_score,
                check_results, alignment_suggestions
            )
    ...
    async def _predict_conflicts(
        ...
        try:
            ...
            if not features or len(features) == 0:
                # 如果没有足够特征，使用简化方法
                return await self._predict_conflicts_simplified(decision_data, related_decisions)
            ...
            try:
                ...
                if self.is_trained:
                    conflict_probability = float(self.rf_classifier.predict_proba(X)[0][1])
                else:
                    # 如果没有训练数据，使用规则方法
                ...
                # 识别具体冲突
                conflicts = await self._identify_conflicts(decision_data, related_decisions)
    ...
    async def _predict_conflicts_simplified(
        ...
        for related in related_decisions:
            ...
            if decision_budget > 0 and related_budget > 0:
                budget_overlap = min(decision_budget, related_budget) / max(decision_budget, related_budget)
    ...
    def _goals_to_features(self, goals_data: List[List[Dict[str, Any]]]) -> Optional[List[Dict[str, float]]]:
        """将目标数据转换为特征矩阵"""
        try:
            feature_list = []
            ...
            for goals in goals_data:
                features = {
                    ...
                    "avg_priority": float(np.mean([g.get("priority", 5) for g in goals])) if goals else 5.0,
                    "has_budget": 1.0 if any(g.get("budget", 0) > 0 for g in goals) else 0.0,
                    "has_timeline": 1.0 if any(g.get("target_date") for g in goals) else 0.0
                }
    ...
    def _calculate_goal_similarity(self, goals_data: List[List[Dict[str, Any]]]) -> List[float]:
        """计算目标相似度"""
        similarities = []
    ...
    def _compute_goals_similarity(self, goals1: List[Dict], goals2: List[Dict]) -> float:
        """计算两组目标的相似度"""
            ...
            for resource_type, amount in decision_resources.items():
                ...
                if amount > 0 and related_amount > 0:
                    conflicts.append({
                        "type": "resource_conflict",
                        "resource_type": resource_type,
                        "conflicting_decisions": [decision_data.get("decision_id"), related.get("decision_id")],
                        "severity": "medium"
                    })
    ...
    def _calculate_alignment_score(self, check_results: Dict[str, Any]) -> float:
        ...
        if "consistency" in check_results:
            consistency_score = check_results["consistency"].get("consistency_score", 0.7)
            scores.append(consistency_score)
        
        return float(np.mean(scores)) if scores else 0.7
    
    def _determine_alignment_status(self, alignment_score: float, check_results: Dict[str, Any]) -> str:
        """确定对齐状态"""
    ...
    async def _save_alignment_check(
        ...
        try:
            ...
            conflict_prob = 0.0
            if "conflicts" in check_results:
                conflict_prob = check_results["conflicts"].get("conflict_probability", 0.0)
            
            # 提取一致性得分
            consistency_score = 0.5
            if "consistency" in check_results:
                consistency_score = check_results["consistency"].get("consistency_score", 0.5)
