...
class ShapleyAttributionRequest(BaseModel):
    ...
    method: Optional[str] = Field(
        "monte_carlo", description="计算方法（exact/monte_carlo）"
    )
...
class ShapleyAttributionResponse(BaseModel):
    ...
    attribution: Dict[str, float] = Field(
        ..., description="归因权重，格式: {'touchpoint_id': weight}"
    )
    method: str
...
async def batch_calculate_shapley_attribution(
    ...
    try:
        ...
        results = service.batch_calculate_attribution(
            orders=request.orders, touchpoint_journey=request.touchpoint_journey
        )
