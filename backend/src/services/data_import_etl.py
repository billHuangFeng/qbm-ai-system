...
class DataImportETL:
    ...
    async def process_data_import(
        ...
        try:
            ...
            if detected_format != document_format:
                logger.warning(
                    f"检测到的格式 {detected_format} 与指定格式 {document_format} 不匹配"
                )

            # 3. 解析数据结构
            parsed_data = await self._parse_document_structure(
                raw_data, document_format
            )
            ...
            load_result = await self._load_data_to_target(
                cleaned_data, target_table, import_config
            )
    ...
    async def _read_source_data(
        self, file_path: str, source_type: DataSourceType
    ) -> Any:
        """读取源数据"""
        try:
    ...
    async def _read_json_file(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"type": "json", "file_path": file_path, "data": data}
    ...
    async def _read_xml_file(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"type": "xml", "file_path": file_path, "data": xml_to_dict(root)}
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(api_endpoint) as response:
                    if response.status == 200:
                        ...
                        return {"type": "api", "endpoint": api_endpoint, "data": data}
                    else:
    ...
    async def _detect_excel_format(self, excel_data: Dict[str, Any]) -> DocumentFormat:
        ...
        try:
            ...
            header_row = data[0]
            has_header = any(
                isinstance(cell, str) and cell.strip() for cell in header_row
            )
    ...
    async def _detect_csv_format(self, csv_data: Dict[str, Any]) -> DocumentFormat:
        ...
        try:
            ...
            first_row = data[0]
            has_header = any(
                isinstance(cell, str) and cell.strip() for cell in first_row
            )
    ...
    async def _parse_document_structure(
        self, raw_data: Dict[str, Any], document_format: DocumentFormat
    ) -> Dict[str, Any]:
        """解析文档结构"""
        try:
            if raw_data["type"] == "excel":
    ...
    async def _parse_excel_structure(
        self, excel_data: Dict[str, Any], document_format: DocumentFormat
    ) -> Dict[str, Any]:
        """解析Excel结构"""
        try:
            parsed_sheets = {}
    ...
    async def _parse_csv_structure(
        self, csv_data: Dict[str, Any], document_format: DocumentFormat
    ) -> Dict[str, Any]:
        """解析CSV结构"""
        try:
            data = csv_data["data"]
    ...
    async def _parse_json_structure(
        self, json_data: Dict[str, Any], document_format: DocumentFormat
    ) -> Dict[str, Any]:
        """解析JSON结构"""
        try:
            data = json_data["data"]
    ...
    async def _apply_field_mappings(
        self, parsed_data: Dict[str, Any], field_mappings: List[FieldMapping]
    ) -> Dict[str, Any]:
        """应用字段映射"""
        try:
    ...
    async def _validate_data(
        self, mapped_data: Dict[str, Any], field_mappings: List[FieldMapping]
    ) -> Dict[str, Any]:
        """验证数据"""
        try:
            ...
            for mapping in field_mappings:
                if mapping.validation_rule:
                    field_validation = await self._validate_field(
                        mapped_data, mapping.target_field, mapping.validation_rule
                    )
    ...
    async def _validate_field(
        self, data: Dict[str, Any], field_name: str, validation_rule: str
    ) -> Dict[str, Any]:
        """验证单个字段"""
        try:
            # 这里实现具体的字段验证逻辑
            ...
            if rule_type == "range":
                ...
                for value in field_data:
                    if isinstance(value, (int, float)) and not (
                        ...
                        result["errors"].append(
                            f"字段 {field_name} 的值 {value} 超出范围 [{min_val}, {max_val}]"
                        )
                ...
                for value in field_data:
                    if isinstance(value, str) and not pattern.match(value):
                        result["valid"] = False
                        result["errors"].append(
                            f"字段 {field_name} 的值 {value} 不符合格式 {rule_value}"
                        )
                ...
                for value in field_data:
                    if str(value) not in allowed_values:
                        result["valid"] = False
                        result["errors"].append(
                            f"字段 {field_name} 的值 {value} 不在允许的枚举值中: {allowed_values}"
                        )
    ...
    async def _clean_data(
        self, mapped_data: Dict[str, Any], validation_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        """清洗数据"""
        try:
            return await self.data_transformer.clean_data(
                mapped_data, validation_result
            )
        except Exception as e:
    ...
    async def _insert_data(
        self, data: Dict[str, Any], target_table: str
    ) -> Dict[str, Any]:
        """插入数据"""
        try:
            # 实现批量插入逻辑
    ...
    async def _update_data(
        self, data: Dict[str, Any], target_table: str, import_config: Dict[str, Any]
    ) -> Dict[str, Any]:
        """更新数据"""
        try:
            # 实现更新逻辑
    ...
    def _load_validation_rules(self) -> List[DataValidationRule]:
        ...
        return [
            ...
            DataValidationRule(
                "status", "enum", "active,inactive,pending", "状态值不在允许范围内"
            ),
        ]
                    ...
                    for i, row in enumerate(rows):
                        for j, cell in enumerate(row):
                            if cell is None or (
                                ...
                                if j < len(headers):
                                    issues.append(
                                        f"工作表 {sheet_name} 第 {i+1} 行字段 {headers[j]} 存在缺失值"
                                    )
                ...
                for sheet_name, sheet_data in data["sheets"].items():
                    ...
                    for i, row in enumerate(rows):
                        ...
                        if row_tuple in seen_rows:
                            issues.append(
                                f"工作表 {sheet_name} 第 {i+1} 行与之前的行完全重复"
                            )
                        else:
                    ...
                    for row in rows:
                        ...
                        for i, cell in enumerate(row):
                            if i < len(headers) and headers[i] in mapping_dict:
                                ...
                                transformed_cell = await self._transform_cell(
                                    cell, mapping
                                )
    ...
    async def _transform_cell(self, cell: Any, mapping: FieldMapping) -> Any:
        ...
        try:
            if mapping.transformation_rule:
                # 应用转换规则
                return await self._apply_transformation_rule(
                    cell, mapping.transformation_rule
                )
            else:
                # 基本类型转换
    ...
    async def _convert_data_type(self, cell: Any, data_type: str) -> Any:
        ...
        try:
            if data_type == "string":
                ...
                return (
                    int(float(cell)) if cell is not None and str(cell).strip() else None
                )
