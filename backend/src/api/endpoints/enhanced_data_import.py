...
async def upload_file(
    ...
    try:
        ...
        if result.success:
            logger.info(
                f"文件上传成功: {result.file_name}, 质量分数: {result.quality_score:.1f}"
            )
        else:
...
async def validate_uploaded_file(
    ...
    try:
        ...
        # 生成建议
        recommendations = data_import_service.generate_recommendations(
            validation_result
        )
        ...
        logger.info(
            f"文件验证完成: {file_name}, 质量分数: {quality_report.quality_score:.1f}"
        )
...
async def create_demo_data(
    rows: int = 100, current_user: User = Depends(get_current_user)
):
    """
    创建演示数据
...
@router.get("/stats")
async def get_import_stats(current_user: User = Depends(get_current_user)):
    """
    获取数据导入统计信息
    ...
    try:
        ...
        recent_imports = [
            h for h in history if datetime.fromisoformat(h["created_time"]) > week_ago
        ]
