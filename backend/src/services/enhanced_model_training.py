...
class ModelTrainingService:
    ...
    def prepare_data(
        ...
        try:
            ...
            y = y.fillna(
                y.mean() if y.dtype in ["int64", "float64"] else y.mode().iloc[0]
            )
    ...
    def train_model(
        ...
        try:
            ...
            logger.info(
                f"模型训练完成: {config.algorithm}, 耗时: {training_time:.2f}秒"
            )
    ...
    def evaluate_model(
        self, model: Any, X_test: pd.DataFrame, y_test: pd.Series, config: ModelConfig
    ) -> Dict[str, Any]:
        """评估模型"""
        try:
            # 预测
    ...
    def cross_validate_model(
        ...
        try:
            scores = cross_val_score(
                ...
                y,
                cv=cv,
                scoring="accuracy" if "classifier" in str(type(model)) else "r2",
            )
    ...
    def save_model(
        self, model: Any, config: ModelConfig, metrics: Dict[str, Any], model_id: str
    ) -> str:
        """保存模型"""
        try:
            # 创建模型目录
    ...
    async def train_complete_model(
        self, df: pd.DataFrame, config: ModelConfig
    ) -> TrainingResult:
        """完整的模型训练流程"""
        try:
...
async def demo_model_training():
    ...
    # 测试模型预测
    print("\n3. 测试模型预测:")
    model, model_info = model_training_service.load_model(
        result_classification.model_id
    )
