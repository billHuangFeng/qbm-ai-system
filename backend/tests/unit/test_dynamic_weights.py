...
class TestDynamicWeightCalculator:
    ...
    def test_calculate_comprehensive_weights(self):
        """测试综合权重计算"""
        weights = self.weight_calculator._calculate_comprehensive_weights(
            self.X, self.y
        )
    ...
    def test_different_objectives(self):
        """测试不同优化目标"""
        # 测试R²目标
        result_r2 = self.weight_calculator.optimize_weights(
            self.X, self.y, objective="r2"
        )
        assert isinstance(result_r2, dict)

        # 测试MSE目标
        result_mse = self.weight_calculator.optimize_weights(
            self.X, self.y, objective="mse"
        )
