...
class ExpertKnowledgeService:
    ...
    async def get_knowledge_by_id(
        self, knowledge_id: str, tenant_id: Optional[str] = None
    ...
    async def update_knowledge(
        self, knowledge_id: str, tenant_id: str, updates: Dict[str, Any]
    ) -> Dict[str, Any]:
        """更新知识"""
        try:
            # 验证更新字段
    ...
    async def delete_knowledge(
        self, knowledge_id: str, tenant_id: str
    ) -> Dict[str, Any]:
        """删除知识（软删除，标记为不活跃）"""
        try:
            if not self.db_service:
    ...
    async def search_knowledge(
        ...
        try:
            ...
            # 查询总数
            count_query = (
                f"SELECT COUNT(*) as total FROM expert_knowledge WHERE {where_clause}"
            )
    ...
    async def get_related_knowledge(
        self, knowledge_id: str, tenant_id: str, limit: int = 5
    ) -> List[Dict[str, Any]]:
        """获取相关知识（基于分类和标签）"""
        try:
            # 先获取当前知识
            ...
            related_list = [
                k
                for k in related.get("knowledge_list", [])
    ...
    async def apply_knowledge(
        ...
        try:
            ...
            await self.db_service.insert(
                "knowledge_application_history", application_data
            )
            ...
            logger.info(
                f"记录知识应用成功: knowledge_id={knowledge_id}, context={application_context}"
            )

            return {"success": True, "application_id": application_data["id"]}
    ...
    async def verify_knowledge(
        ...
        try:
            ...
            update_data = {
                ...
                "verified_at": (
                    datetime.now() if verification_status != "pending" else None
                ),
                "updated_at": datetime.now(),
            }
            ...
            logger.info(
                f"验证知识成功: knowledge_id={knowledge_id}, status={verification_status}"
            )
    ...
    async def _get_attachments(self, knowledge_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            attachments = await self.db_service.fetch_all(
    ...
    async def _update_application_stats(self, knowledge_id: str):
        ...
        try:
            ...
            """
            stats = await self.db_service.fetch_one(
                query, {"knowledge_id": knowledge_id}
            )
            ...
            if stats:
                ...
                success_rate = (
                    (successful_count / applied_count) if applied_count > 0 else None
                )
