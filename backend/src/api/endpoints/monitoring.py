...
async def get_monitoring_data(
    ...
    try:
        ...
        monitoring_data = []
        for row in rows:
            monitoring_data.append(MonitoringResponse(
                id=row[0],
                tenant_id=row[1],
                model_id=row[2],
                metric_name=row[3],
...
async def get_system_health(
    ...
    try:
        ...
        # 判断整体健康状态
        unhealthy_services = [service for service, status in health_status["services"].items() if "unhealthy" in status]
        if unhealthy_services:
            health_status["status"] = "degraded"
        if cpu_percent > 90 or (memory_percent and memory_percent > 90) or (disk_percent and disk_percent > 90):
...
async def get_performance_metrics(
    ...
    try:
        ...
        metrics["model_training"] = {
            ...
            "average_accuracy": round(training_stats[1], 4) if training_stats and training_stats[1] else 0,
            "average_training_time_seconds": round(training_stats[2], 2) if training_stats and training_stats[2] else 0
        }
        ...
        prediction_query = """
            ...
            WHERE tenant_id = $1 AND created_at >= $2
        """
        result = await db.execute(prediction_query, [current_user.tenant_id, start_time])
        prediction_stats = result.fetchone()
        
        metrics["predictions"] = {
            "total_predictions": prediction_stats[0] if prediction_stats else 0,
            "average_prediction_time_seconds": round(prediction_stats[1], 3) if prediction_stats and prediction_stats[1] else 0,
            "average_confidence": round(prediction_stats[2], 4) if prediction_stats and prediction_stats[2] else 0
        }
        ...
        metrics["data_import"] = {
            ...
            "average_processing_time_seconds": round(import_stats[2], 2) if import_stats and import_stats[2] else 0
        }
        ...
        metrics["api_calls"] = {
            ...
            "average_response_time_ms": round(api_stats[2], 2) if api_stats and api_stats[2] else 0
        }
        ...
        metrics["data_quality"] = {
            "average_score": round(quality_stats[0], 2) if quality_stats and quality_stats[0] else 0,
            "total_checks": quality_stats[1] if quality_stats else 0
        }
        
        logger.info(f"Performance metrics retrieved for tenant: {current_user.tenant_id}")
        return metrics
