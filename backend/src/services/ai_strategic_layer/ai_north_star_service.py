...
class AINorthStarService:
    ...
    async def create_north_star_metric(
        ...
        **kwargs
        ...
        try:
            ...
            if self.db_service:
                metric_id = await self.db_service.execute_insert(
                    ...
                    [
                        metric_name, metric_description, metric_code, strategic_objective_id,
                        metric_type, target_value, calculation_formula, measurement_frequency,
                        "active", kwargs.get("is_primary", False)
                    ]
                )
            else:
            ...
            # 2. AI优化权重
            ai_weight = await self._calculate_ai_weight(metric_id, strategic_objective_id)
        ...
        try:
            ...
            if len(related_metrics) > 1 and history_data:
                try:
                    ...
                    if not history_df.empty:
                        history_df['measurement_date'] = pd.to_datetime(history_df['measurement_date'])
                        ...
                        if len(pivot_df.columns) > 1 and len(pivot_df) >= 10:
                            ...
                            for col in pivot_df.columns:
                                if col in metric_info:
                                    ...
                                    if target_val is not None and current_val is not None:
                                        # 目标变量：目标与当前值的差距（差距越大，越重要）
                                        importance = abs(target_val - current_val) if target_val != 0 else abs(current_val)
                                        target_data.append(importance)
                                        feature_columns.append(col)
                            
                            if len(feature_columns) > 1 and len(target_data) == len(feature_columns):
                                # 创建特征矩阵（使用最近N期的平均值）
                                X_data = pivot_df[feature_columns].ffill().bfill().fillna(0)
                                ...
                                if len(X_data) >= 10:
                                    ...
                                    # 创建DataFrame和Series
                                    X = pd.DataFrame([X_avg.values], columns=feature_columns)
                                    ...
                                    weight_results = self.weight_calculator.calculate_dynamic_weights(
                                        X, y, method='comprehensive', update_frequency='monthly'
                                    )
                                    ...
                                    if final_weights and metric_id in final_weights:
                                        ...
                                        effectiveness = weight_results.get('effectiveness', {})
                                        
                                        return {
                                            "weight": float(metric_weight),
                                            "priority": min(10, max(1, int(metric_weight * 20))),  # 转换为1-10的优先级
                                            "calculation_method": "dynamic_weights_comprehensive",
                                            "weight_details": {
                                                "correlation": normalized_weights.get('correlation', {}).get(metric_id, 0),
                                                "importance": normalized_weights.get('importance', {}).get(metric_id, 0),
                                                "regression": normalized_weights.get('regression', {}).get(metric_id, 0),
                                                "time_series": normalized_weights.get('time_series', {}).get(metric_id, 0)
                                            },
                                            "effectiveness": {
                                                "r2_improvement": effectiveness.get('r2_improvement', 0),
                                                "effectiveness_score": effectiveness.get('effectiveness_score', 0)
                                            },
                                            "stability": weight_results.get('stability', {}),
                                            "timestamp": datetime.now().isoformat()
                                        }
                
                except Exception as e:
                    logger.warning(f"使用DynamicWeightCalculator计算权重失败: {e}，回退到简化方法")
            ...
            if len(related_metrics) > 1:
                ...
                for metric in related_metrics:
                    if metric.get("target_value") is not None and metric.get("current_value") is not None:
                        metric_data.append({
                        ...
                        # 目标差距越大，权重越高
                        gap = abs(metric.get("target_value", 0) - metric.get("current_value", 0))
                        # 归一化：使用相对差距
    ...
    async def _predict_trend(self, metric_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if best_arima:
                    # 预测未来3个周期
                    forecast, lower, upper = best_arima.predict_interval(steps=3, alpha=0.05)
                    ...
                    return {
                        ...
                        "predicted_values": forecast.tolist() if hasattr(forecast, 'tolist') else list(forecast),
                ...
                if len(time_series) >= 2:
                    ...
                    return {
                        "prediction_method": "insufficient_data",
                        "predicted_value": float(time_series[-1]) if len(time_series) > 0 else None,
                        "confidence": 0.3,
                        "forecast_periods": 3
                    }
            ...
            if self.memory_service:
                try:
                    patterns = await self.memory_service.search_similar_patterns(
                        query=f"strategic objective {strategic_objective_id}",
                        limit=10
                    )
                    for pattern in patterns[:5]:
                        pattern_dict = pattern.dict() if hasattr(pattern, 'dict') else pattern
                        recommendations.append({
                            "metric_name": pattern_dict.get("description", "推荐指标"),
    ...
    def _get_standard_metrics_by_type(self, objective_type: str) -> List[Dict[str, Any]]:
        """根据目标类型获取标准指标"""
        metric_templates = {
            "mission": [
                {
            ...
            if self.db_service:
                ...
                if metric:
                    ai_weight = await self._calculate_ai_weight(
                        metric_id,
                        metric.get("strategic_objective_id")
                    )
                    trend_prediction = await self._predict_trend(metric_id)
    ...
    async def calculate_metric_health_score(self, metric_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if target_value is not None and target_value != 0:
                achievement_rate = min(1.0, abs(current_value / target_value)) if target_value > 0 else 0
                ...
                if trend.get("prediction_method") == "arima":
                    ...
                    if predicted_values and len(predicted_values) > 0:
                        ...
                        if target_value and current_value:
                            ...
                            # 趋势方向与目标方向一致为健康
                            if (target_gap > 0 and trend_direction > 0) or (target_gap < 0 and trend_direction < 0):
            ...
            if history_data and len(history_data) >= 10:
                ...
                health_factors["data_completeness"] = len(history_data) / 10 if history_data else 0
            ...
            return {
                ...
                "weights": weights,
                "recommendations": self._generate_health_recommendations(health_factors, status)
            }
        ...
        try:
            ...
            comparison = {
                ...
                "comparison_summary": {
                    ...
                    "avg_health_score": float(np.mean([m["health_score"]["health_score"] for m in metrics_info])),
                    "best_metric": max(metrics_info, key=lambda x: x["health_score"]["health_score"]),
                    "worst_metric": min(metrics_info, key=lambda x: x["health_score"]["health_score"])
                },
    ...
    def _generate_comparison_insights(self, metrics_info: List[Dict[str, Any]]) -> List[str]:
        """生成对比洞察"""
        insights = []
        ...
        if len(metrics_info) >= 2:
            ...
            # 分析趋势
            trend_statuses = [m["health_score"].get("factors", {}).get("trend", 0.5) for m in metrics_info]
