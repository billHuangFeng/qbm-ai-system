...
class ModelTrainingService:
    ...
    def train_marginal_analysis_model(
        ...
        try:
            ...
            results["random_forest"] = {"model": rf, "scores": rf_scores}
            ...
            results["xgboost"] = {"model": xgb_model, "scores": xgb_scores}
            ...
            results["lightgbm"] = {"model": lgb_model, "scores": lgb_scores}

            # 选择最佳模型
            best_model_name = min(
                results.keys(), key=lambda k: results[k]["scores"]["mae"]
            )
    ...
    def train_timeseries_model(
        ...
        try:
            ...
            feature_columns = [
                ...
                if col != target_variable and col != date_column
            ]
            ...
            predictions = self._generate_forecast(
                model, data, feature_columns, forecast_periods
            )
            ...
            return {
                ...
                "scores": {"mae": cv_mae, "cv_std": cv_scores.std()},
                "feature_importance": feature_importance,
    ...
    def _generate_forecast(
        self, model: Any, data: pd.DataFrame, feature_columns: List[str], periods: int
    ) -> List[Dict[str, Any]]:
        """生成预测"""
        predictions = []
            ...
            predictions.append(
                {
                    ...
                    "date": (datetime.now() + pd.DateOffset(months=i)).strftime(
                        "%Y-%m"
                    ),
                }
            )
        ...
        try:
            if update_strategy == "incremental":
                ...
                updated_data = pd.concat(
                    [training_data, feedback_df], ignore_index=True
                )
    ...
    def evaluate_model_performance(
        ...
        try:
            ...
            return {"success": True, "scores": scores, "data_size": len(test_data)}
    ...
    async def predict(
        self, model_id: str, input_data: Dict[str, Any], tenant_id: str
    ) -> Dict[str, Any]:
        """
        使用模型进行预测
    ...
    def train_npv_model(
        ...
        try:
            ...
            merged_data = pd.merge(
                asset_data, cash_flow_data, on="asset_id", how="inner"
            )
    ...
    def train_capability_value_model(
        ...
        try:
            # 合并数据
            merged_data = pd.merge(
                capability_data, performance_data, on="capability_id", how="inner"
            )
        ...
        try:
            ...
            model_data = {
                ...
                "parameters": {
                    "model": model_base64,
                    "feature_list": list(
                        training_result.get("feature_importance", {}).keys()
                    ),
                },
            ...
            # 保存到数据库
            result = (
                supabase_client.table("model_parameters_storage")
                .insert(model_data)
                .execute()
            )
        ...
        for column in X_encoded.columns:
            if X_encoded[column].dtype == "object":
                if column not in self.label_encoders:
                    ...
                    X_encoded[column] = self.label_encoders[column].fit_transform(
                        X_encoded[column]
                    )
                else:
    ...
    def _preprocess_input_data(
        self, X: pd.DataFrame, model_info: Dict[str, Any]
    ) -> pd.DataFrame:
        """预处理输入数据"""
        # 处理缺失值
        X = X.fillna(X.mean())
        ...
        if numeric_columns.any():
            ...
            if scaler_key not in self.scalers:
                ...
                X[numeric_columns] = self.scalers[scaler_key].fit_transform(
                    X[numeric_columns]
                )
            else:
                X[numeric_columns] = self.scalers[scaler_key].transform(
                    X[numeric_columns]
                )
    ...
    def _calculate_confidence_interval(
        self, model: Any, X: pd.DataFrame, mae: float, confidence_level: float = 0.95
    ) -> Dict[str, float]:
        """计算置信区间"""
        try:
            # 使用MAE估算标准差
