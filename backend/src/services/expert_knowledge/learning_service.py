...
class LearningService:
    ...
    async def create_course(
        ...
        try:
            ...
            return {"success": True, "course_id": course_id, "course": course_data}
    ...
    async def get_course_by_id(
        self, course_id: str, tenant_id: Optional[str] = None
    ...
    async def list_courses(
        ...
        try:
            ...
            # 查询总数
            count_query = (
                f"SELECT COUNT(*) as total FROM learning_courses WHERE {where_clause}"
            )
    ...
    async def create_learning_path(
        ...
        try:
            ...
            return {"success": True, "path_id": path_id, "path": path_data}
        ...
        try:
            ...
            if existing:
                ...
                await self._update_learning_record(
                    existing["id"], {"last_accessed_at": datetime.now()}
                )
                return {
                    "success": True,
    ...
    async def update_learning_progress(
        ...
        try:
            ...
            updates = {"last_accessed_at": datetime.now(), "updated_at": datetime.now()}
            ...
            if time_spent_minutes is not None:
                updates["time_spent_minutes"] = (
                    record.get("time_spent_minutes", 0) + time_spent_minutes
                )
    ...
    async def submit_exercise_answer(
        self, exercise_id: str, user_id: str, answer: Any
    ) -> Dict[str, Any]:
        """提交练习答案"""
        try:
            if not self.db_service:
            ...
            is_correct = self._check_answer(
                answer, correct_answer, exercise.get("exercise_type")
            )
            ...
            return {
                ...
                "correct_answer": (
                    correct_answer if is_correct else None
                ),  # 答对才显示答案
            }
    ...
    async def submit_test(
        ...
        try:
            ...
            test = await self.db_service.fetch_one(
                "SELECT * FROM learning_tests WHERE id = :id", {"id": test_id}
            )
            ...
            for ex_id in exercise_ids:
                ex = await self.db_service.fetch_one(
                    "SELECT * FROM learning_exercises WHERE id = :id", {"id": ex_id}
                )
                if not ex:
                    continue
                ...
                is_correct = self._check_answer(
                    user_answer, correct_answer, ex.get("exercise_type")
                )
    ...
    async def _update_learning_record(self, record_id: str, updates: Dict[str, Any]):
        ...
        try:
            ...
            await self.db_service.update("learning_records", {"id": record_id}, updates)

        except Exception as e:
            logger.error(f"更新学习记录失败: {e}")

    def _check_answer(
        self, user_answer: Any, correct_answer: Any, exercise_type: str
    ) -> bool:
        """检查答案是否正确"""
        try:
