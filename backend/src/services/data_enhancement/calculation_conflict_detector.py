...
class CalculationConflictDetector(BaseService):
    ...
    def detect_conflicts_in_record(
        ...
        for rule in calculation_rules:
            try:
                ...
                if not is_match:
                    # 判断严重程度
                    relative_diff = abs(difference) / abs(expected_decimal) if expected_decimal != 0 else float('inf')
                    ...
                    # 生成修复建议
                    suggested_fix = "use_calculated_value" if auto_fixable else "manual_review"
    ...
    def detect_cascade_conflicts(
        ...
        for record in records:
            ...
            for conflict in conflicts:
                ...
                # 查找依赖该字段的其他字段
                dependent_fields = [
                    field for field, deps in dependency_graph.items()
    ...
    async def detect_conflicts(
        ...
        try:
            ...
            auto_fixable_count = sum(1 for c in all_conflicts if c.get("auto_fixable", False))
