...
class TestAPIPerformance:
            ...
            with patch(
                ...
                with patch(
                    ...
                    mock_psutil.virtual_memory.return_value = Mock(
                        percent=60.0, available=8 * 1024**3
                    )
                    mock_psutil.disk_usage.return_value = Mock(
                        percent=45.0, free=120 * 1024**3
                    )
    ...
    async def test_concurrent_requests_performance(self, mock_user):
        ...
        assert (
            avg_time_per_request < 0.2
        ), f"并发请求平均响应时间过长: {avg_time_per_request:.3f}s"
...
class TestScalability:
    ...
    async def test_response_time_scalability(self):
        ...
        for i in range(1, len(response_times)):
            ...
            # 响应时间增长不应该超过并发增长的2倍
            assert (
                time_ratio < concurrency_ratio * 2
            ), f"响应时间增长过快: {time_ratio:.2f} vs {concurrency_ratio:.2f}"
