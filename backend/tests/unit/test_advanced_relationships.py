...
class TestAdvancedRelationshipAnalysis:
    ...
    def test_identify_advanced_relationships(self):
        """测试高级关系识别"""
        result = self.relationship_analysis.identify_advanced_relationships(
            self.X, self.y
        )
    ...
    def test_analyze_ensemble_relationships(self):
        """测试集成模型关系分析"""
        result = self.relationship_analysis._analyze_ensemble_relationships(
            self.X, self.y, 5
        )
    ...
    def test_analyze_neural_relationships(self):
        """测试神经网络关系分析"""
        result = self.relationship_analysis._analyze_neural_relationships(
            self.X, self.y, 5
        )
    ...
    def test_analyze_feature_interactions(self):
        """测试特征交互关系分析"""
        result = self.relationship_analysis._analyze_feature_interactions(
            self.X, self.y
        )
    ...
    def test_analyze_pairwise_interactions(self):
        """测试两两特征交互分析"""
        result = self.relationship_analysis._analyze_pairwise_interactions(
            self.X, self.y
        )
        ...
        if result["all_interactions"]:
            for interaction_name, interaction_data in result[
                "all_interactions"
            ].items():
    ...
    def test_analyze_feature_combinations(self):
        """测试特征组合分析"""
        result = self.relationship_analysis._analyze_feature_combinations(
            self.X, self.y
        )
    ...
    def test_analyze_nonlinear_relationships(self):
        """测试非线性关系分析"""
        result = self.relationship_analysis._analyze_nonlinear_relationships(
            self.X, self.y, 5
        )
    ...
    def test_analyze_log_transformations(self):
        ...
        result = self.relationship_analysis._analyze_log_transformations(
            positive_X, positive_y, 5
        )
    ...
    def test_analyze_exponential_transformations(self):
        """测试指数变换分析"""
        result = self.relationship_analysis._analyze_exponential_transformations(
            self.X, self.y, 5
        )
    ...
    def test_calculate_relationship_score(self):
        ...
        relationship_results = {
            ...
            "feature_interactions": {
                ...
                "triple": {
                    "top_triple_interactions": [("f1_x_f2_x_f3", {"significance": 0.3})]
                },
            },
            "nonlinear": {"model1": {"mean_cv_score": 0.5}},
        }

        score = self.relationship_analysis._calculate_relationship_score(
            relationship_results
        )
    ...
    def test_get_relationship_insights(self):
        ...
        # 检查关系水平
        assert insights["relationship_level"] in [
            "高复杂度关系",
            "中等复杂度关系",
            "低复杂度关系",
            "简单线性关系",
    ...
    def test_classify_relationship_level(self):
        """测试关系水平分类"""
        assert (
            self.relationship_analysis._classify_relationship_level(0.9)
            == "高复杂度关系"
        )
        assert (
            self.relationship_analysis._classify_relationship_level(0.7)
            == "中等复杂度关系"
        )
        assert (
            self.relationship_analysis._classify_relationship_level(0.4)
            == "低复杂度关系"
        )
        assert (
            self.relationship_analysis._classify_relationship_level(0.2)
            == "简单线性关系"
        )
    ...
    def test_extract_key_relationships(self):
        ...
        self.relationship_analysis.relationship_insights = {
            ...
            "neural_network": {"mlp_deep": {"mean_cv_score": 0.75}},
            "feature_interactions": {
                "pairwise": {"top_interactions": [("f1_x_f2", {"significance": 0.6})]}
            },
        }
    ...
    def test_generate_relationship_recommendations(self):
        ...
        recommendations_high = (
            self.relationship_analysis._generate_relationship_recommendations()
        )
    ...
    def test_clear_nonlinear_data(self):
        ...
        nonlinear_X = pd.DataFrame(
            {"feature1": np.linspace(-2, 2, 100), "feature2": np.linspace(-2, 2, 100)}
        )
    ...
    def test_high_dimensional_data(self):
        ...
        high_dim_X = pd.DataFrame(
            {f"feature_{i}": np.random.normal(0, 1, 100) for i in range(20)}
        )
        high_dim_y = pd.Series(np.random.normal(0, 1, 100))

        result = self.relationship_analysis.identify_advanced_relationships(
            high_dim_X, high_dim_y
        )
