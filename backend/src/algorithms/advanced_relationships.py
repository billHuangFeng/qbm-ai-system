...
class AdvancedRelationships:
    ...
    def analyze_complex_relationships(
        self, X: pd.DataFrame, y: pd.Series, relationship_types: List[str] = None
    ) -> Dict[str, Any]:
        """分析复杂关系"""
        try:
    ...
    def _analyze_linear_relationships(
        ...
        try:
            ...
            linear_results["multiple_linear"] = {
                ...
                "feature_importance": dict(
                    zip(X.columns, np.abs(model_multiple.coef_))
                ),
            }
            ...
            for degree in [2, 3]:
                ...
                for feature in X.columns:
                    ...
                    X_poly = np.column_stack(
                        [X_feature**i for i in range(1, degree + 1)]
                    )
    ...
    def _analyze_interaction_relationships(
        self, X: pd.DataFrame, y: pd.Series
    ) -> Dict[str, Any]:
        """分析交互关系"""
        try:
            ...
            for i, feature1 in enumerate(features):
                for j, feature2 in enumerate(features[i + 1 :], i + 1):
                    ...
                    pairwise_interactions[f"{feature1}_x_{feature2}"] = {
                        ...
                        "effect_size": abs(interaction_coef)
                        * np.std(interaction_feature),
                    }
            ...
            logger.info(
                f"交互关系分析完成，发现 {len(pairwise_interactions)} 个两两交互"
            )
    ...
    def _analyze_hierarchical_relationships(
        self, X: pd.DataFrame, y: pd.Series
    ) -> Dict[str, Any]:
        """分析层次关系"""
        try:
    ...
    def _detect_nonlinear_patterns(
        self, X: pd.DataFrame, y: pd.Series
    ) -> Dict[str, Any]:
        """检测非线性模式"""
        try:
            ...
            for feature in X.columns:
                ...
                # 1. 分段线性模式
                piecewise_pattern = self._detect_piecewise_pattern(
                    feature_values, y_values
                )
                ...
                # 3. 阈值模式
                threshold_pattern = self._detect_threshold_pattern(
                    feature_values, y_values
                )
    ...
    def _analyze_higher_order_interactions(
        self, X: pd.DataFrame, y: pd.Series
    ) -> Dict[str, Any]:
        """分析高阶交互"""
        try:
    ...
    def _analyze_conditional_interactions(
        self, X: pd.DataFrame, y: pd.Series
    ) -> Dict[str, Any]:
        """分析条件交互"""
        try:
    ...
    def _build_feature_hierarchy(self, X: pd.DataFrame, y: pd.Series) -> Dict[str, Any]:
        ...
        try:
            ...
            hierarchy = {
                ...
                "level_2": sorted_features[
                    len(sorted_features) // 3 : 2 * len(sorted_features) // 3
                ],  # 中重要性
    ...
    def _analyze_decision_hierarchy(
        self, X: pd.DataFrame, y: pd.Series
    ) -> Dict[str, Any]:
        """分析决策层次"""
        try:
                ...
                if len(x) > lag:
                    ...
                    if len(x_lagged) > 10:
                        ...
                        X_unrestricted = np.column_stack(
                            [
                                x_lagged,
                                y_current[:-1] if len(y_current) > 1 else x_lagged[:-1],
                            ]
                        )
                        y_unrestricted = (
                            y_current[1:] if len(y_current) > 1 else y_current[1:]
                        )

                        if len(X_unrestricted) > 5:
                            model_unrestricted = LinearRegression()
                            model_unrestricted.fit(X_unrestricted, y_unrestricted)
                            r2_unrestricted = model_unrestricted.score(
                                X_unrestricted, y_unrestricted
                            )

                            # 约束模型
                            X_restricted = (
                                y_current[:-1].reshape(-1, 1)
                                if len(y_current) > 1
                                else x_lagged[:-1].reshape(-1, 1)
                            )
                            y_restricted = (
                                y_current[1:] if len(y_current) > 1 else y_current[1:]
                            )
                            ...
                            if len(X_restricted) > 5:
                                ...
                                r2_restricted = model_restricted.score(
                                    X_restricted, y_restricted
                                )

                                # F统计量
                                n = len(y_unrestricted)
                                k = X_unrestricted.shape[1]
                                f_stat = ((r2_unrestricted - r2_restricted) / 1) / (
                                    (1 - r2_unrestricted) / (n - k)
                                )
            ...
            for feature in X.columns:
                ...
                if len(other_features) > 0:
                    ...
                    iv_results[feature] = {
                        ...
                        "stage2_r2": model_stage2.score(
                            y_pred_stage1.reshape(-1, 1), y
                        ),
    ...
    def _create_matched_pairs(
        self, propensity_scores: np.ndarray, y_binary: np.ndarray
    ) -> Dict[str, Any]:
        """创建匹配对"""
        try:
            # 简化的匹配算法
            ...
            return {
                ...
                "avg_distance": (
                    np.mean([pair["distance"] for pair in matched_pairs])
                    if matched_pairs
                    else 0
                ),
            }
    ...
    def _calculate_interaction_significance(
        self, x1: pd.Series, x2: pd.Series, interaction: pd.Series, y: pd.Series
    ) -> float:
        """计算交互项显著性"""
        try:
            # 使用t检验计算交互项的显著性
            X_interaction = np.column_stack([x1.values, x2.values, interaction.values])
    ...
    def _calculate_relationship_score(self, results: Dict[str, Any]) -> float:
        ...
        try:
            ...
            if "hierarchical" in results and results["hierarchical"]:
                hierarchical_score = self._calculate_hierarchical_score(
            ...
            if "pairwise" in interaction_results:
                ...
                if pairwise_interactions:
                    avg_r2 = np.mean(
                        [
                            interaction["r2"]
                            for interaction in pairwise_interactions.values()
                        ]
                    )
    ...
    def _calculate_hierarchical_score(
        self, hierarchical_results: Dict[str, Any]
    ) -> float:
        """计算层次关系评分"""
        try:
    ...
    def create_relationship_features(
        self, X: pd.DataFrame, relationship_threshold: float = 0.1
    ) -> pd.DataFrame:
        """创建关系特征"""
        try:
            X_enhanced = X.copy()
            ...
            if "interaction" in self.relationship_scores:
                ...
                if "pairwise" in interaction_results:
                    for interaction_name, interaction_data in interaction_results[
                        "pairwise"
                    ].items():
                        if interaction_data["r2"] > relationship_threshold:
                            ...
                            # 创建交互特征
                            X_enhanced[f"{feature1}_x_{feature2}"] = (
                                X[feature1] * X[feature2]
                            )

                            # 创建比率特征
                            if X[feature2].min() > 0:
                                X_enhanced[f"{feature1}_div_{feature2}"] = (
                                    X[feature1] / X[feature2]
                                )
            ...
            if "nonlinear" in self.relationship_scores:
                ...
                if "patterns" in nonlinear_results:
                    for pattern_name, pattern_data in nonlinear_results[
                        "patterns"
                    ].items():
                        if (
                            pattern_data.get("type") == "threshold"
                            and pattern_data.get("r2", 0) > relationship_threshold
                        ):
                            feature_name = pattern_name.split("_threshold")[0]
                            threshold = pattern_data["threshold"]

                            # 创建阈值特征
                            X_enhanced[f"{feature_name}_above_threshold"] = (
                                X[feature_name] > threshold
                            ).astype(int)

            logger.info(
                f"关系特征创建完成，新增 {len(X_enhanced.columns) - len(X.columns)} 个特征"
            )
            return X_enhanced
    ...
    def get_relationship_insights(self) -> Dict[str, Any]:
        ...
        try:
            insights = {
                "overall_relationship_score": self.relationship_scores.get(
                    "overall_score", 0
                ),
                            ...
                            key_relationships.append(
                                {
                                    "type": "interaction",
                                    "features": [
                                        interaction_data["feature1"],
                                        interaction_data["feature2"],
                                    ],
                ...
                if "granger" in causal_results:
                    for feature, granger_data in causal_results["granger"].items():
                        for lag_name, lag_data in granger_data.items():
                            if isinstance(lag_data, dict) and lag_data.get(
                                "causality", False
                            ):
                                key_relationships.append(
                                    {
