...
class KnowledgeIntegrationService:
    ...
    async def search_relevant_knowledge(
        self, tenant_id: str, context: Dict[str, Any], limit: int = 5
    ) -> List[Dict[str, Any]]:
        """在AI决策时搜索相关知识"""
        try:
            # 从上下文中提取信息
            ...
            description = context.get("description") or context.get(
    ...
    async def apply_knowledge_to_decision(
        ...
        try:
            # 获取知识详情
            knowledge = await self.knowledge_service.get_knowledge_by_id(
            ...
            # 记录应用
            applied_content = (
                f"{knowledge.get('title', '')}: {knowledge.get('summary', '')[:200]}"
            )
            ...
            logger.info(
                f"知识应用成功: knowledge_id={knowledge_id}, type={application_type}"
            )
    ...
    async def combine_with_enterprise_memory(
        ...
        try:
            ...
            expert_knowledge = await self.search_relevant_knowledge(
                tenant_id=tenant_id, context=context, limit=expert_knowledge_limit
            )
            ...
            if self.memory_service:
                try:
                    # 构建搜索查询
                    query = context.get("description", "") or context.get(
                        "requirement_description", ""
                    )
                    if not query:
                    ...
                    if hasattr(self.memory_service, "search_similar_patterns"):
                        memory_results = (
                            await self.memory_service.search_similar_patterns(
    ...
    async def generate_reasoning_chain(
        ...
        try:
            ...
            # 1. 搜索专家知识（理论框架）
            expert_knowledge = await self.search_relevant_knowledge(
                tenant_id=tenant_id, context=decision_context, limit=3
            )
            ...
            if self.memory_service:
                ...
                reasoning_chain["enterprise_memory"] = combined.get(
                    "enterprise_memory", []
                )
            ...
            if reasoning_chain.get("enterprise_memory"):
                for memory in reasoning_chain["enterprise_memory"][:2]:  # 取前2个
                    memory_desc = memory.get("description", "") or memory.get(
                        "memory_description", ""
                    )
                    reasoning_steps.append(
                        {
    ...
    def _generate_conclusion(
        ...
        conclusion["summary"] = (
            "；".join(summary_parts) if summary_parts else "基于现有信息生成决策建议"
        )
        ...
        if expert_knowledge:
            ...
            verified_count = sum(
                ...
                if k.get("verification_status") == "verified"
            )
