...
class StagingTableManager(BaseService):
    ...
    def generate_staging_table_name(self, data_type: str, tenant_id: str) -> str:
        """
        生成暂存表名称
    ...
    async def get_target_table_schema(
        self, target_table: str, tenant_id: str
    ) -> List[Dict[str, Any]]:
        """
        获取目标表的结构
        ...
        try:
            ...
            results = await self.db_service.execute_query(
                query, params=[schema_name, target_table], fetch_all=True
            )
    ...
    async def insert_to_staging(
        ...
        try:
            if not records:
                return {"row_count": 0, "status": "success"}
            ...
            for record in df.to_dict("records"):
                ...
                await self.db_service.execute_query(insert_sql, params=values)
                inserted_count += 1
    ...
    async def migrate_to_target(
        ...
        try:
            ...
            count_result = await self.db_service.execute_query(
                count_query, params=[tenant_id], fetch_one=True
            )
            ...
            target_columns = [
                col["column_name"]
                for col in schema
                if col["column_name"] not in ["created_at", "tenant_id", "status"]
            ]
            ...
            # 执行迁移（使用事务）
            operations = [
                {"query": migrate_sql, "params": [tenant_id]},
                {
                    "query": f"""
            ...
            logger.info(
                f"从暂存表 {staging_table_name} 迁移 {row_count} 条记录到 {target_table}"
            )
    ...
    async def cleanup_staging_tables(
        ...
        try:
            ...
            staging_tables = await self.db_service.execute_query(
                query, params=[schema_name], fetch_all=True
            )
            ...
            for table_info in staging_tables:
                ...
                result = await self.db_service.execute_query(
                    check_query, params=[tenant_id], fetch_one=True
                )
        ...
        try:
            if operation == "create":
                ...
                result = await self.create_staging_table(
                    data_type, target_table, tenant_id
                )
                ...
                if records:
                    insert_result = await self.insert_to_staging(
                        staging_table_name, schema_name, records, tenant_id
                    )
                ...
                if not staging_table_name or not target_table:
                    raise StagingTableError(
                        "migrate操作需要staging_table_name和target_table参数"
                    )

                schema_name = f"tenant_{tenant_id.replace('-', '_')}"

                return await self.migrate_to_target(
                    staging_table_name, schema_name, target_table, tenant_id
                )

            elif operation == "cleanup":
                return await self.cleanup_staging_tables(tenant_id, retention_days)
