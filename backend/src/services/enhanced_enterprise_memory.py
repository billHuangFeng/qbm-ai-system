...
class EnterpriseMemoryService:
        ...
        try:
            if target_column and target_column in df.columns:
                ...
                if df[target_column].dtype in ["int64", "float64"]:
                    ...
                    # 分类型目标：特定类别表示成功
                    success_data = df[
                        df[target_column] == df[target_column].mode().iloc[0]
                    ]
                ...
                if len(success_data) > 0:
                    ...
                    for col in df.columns:
                        if col != target_column and df[col].dtype in [
                            ...
                            if (
                                success_mean > overall_mean * 1.2
                            ):  # 成功案例中该特征值明显更高
                                pattern = KnowledgePattern(
                                    pattern_id=self._generate_id(
                                        f"success_{col}_{success_mean}"
                                    ),
        ...
        try:
            if target_column and target_column in df.columns:
                ...
                if df[target_column].dtype in ["int64", "float64"]:
                    ...
                    # 分类型目标：特定类别表示失败
                    failure_data = df[
                        df[target_column] == df[target_column].value_counts().index[-1]
                    ]
                ...
                if len(failure_data) > 0:
                    ...
                    for col in df.columns:
                        if col != target_column and df[col].dtype in [
                            ...
                            if (
                                failure_mean < overall_mean * 0.8
                            ):  # 失败案例中该特征值明显更低
                                pattern = KnowledgePattern(
                                    pattern_id=self._generate_id(
                                        f"failure_{col}_{failure_mean}"
                                    ),
                    ...
                    for j, col2 in enumerate(numeric_cols):
                        if i < j:  # 避免重复
                            ...
                            if abs(corr) > 0.7:  # 强相关
                                pattern = KnowledgePattern(
                                    pattern_id=self._generate_id(
                                        f"trend_{col1}_{col2}_{corr}"
                                    ),
                                    pattern_type="trend",
                                    description=f"{col1}与{col2}呈{'正' if corr > 0 else '负'}相关",
                                    conditions={col1: {"correlation": corr}},
                                    outcomes={
                                        "prediction": f"当{col1}变化时，{col2}会{'同向' if corr > 0 else '反向'}变化"
                                    },
                                    confidence=abs(corr),
                                    frequency=len(df),
                                    last_seen=datetime.now(),
    ...
    async def _identify_anomaly_patterns(
        ...
        try:
            ...
            for col in numeric_cols:
                ...
                if (
                    len(anomalies) > 0 and len(anomalies) < len(df) * 0.1
                ):  # 异常值不超过10%
                    pattern = KnowledgePattern(
    ...
    async def generate_business_insights(
        self, patterns: List[KnowledgePattern], context: Dict[str, Any] = None
                ...
                for pattern in trend_patterns:
                    insight = BusinessInsight(
                        insight_id=self._generate_id(
                            f"efficiency_{pattern.pattern_id}"
                        ),
                        category="efficiency",
                        title=f"发现效率优化机会",
        ...
        try:
            ...
            if success_patterns:
                ...
                opportunity_score = sum(
                    p.confidence * p.frequency for p in success_patterns
                )

                if opportunity_score > 0:
                    insight = BusinessInsight(
                        insight_id=self._generate_id(
                            f"opportunity_{opportunity_score}"
                        ),
    ...
    async def generate_recommendations(
        self, insights: List[BusinessInsight], current_context: Dict[str, Any] = None
    ) -> List[Recommendation]:
        """生成智能推荐"""
        try:
    ...
    async def _generate_performance_recommendation(
        self, insight: BusinessInsight
    ...
    async def save_recommendation(self, recommendation: Recommendation) -> None:
        ...
        try:
            file_path = (
                self.recommendations_dir / f"{recommendation.recommendation_id}.json"
            )
            self._save_to_file(recommendation, file_path)
            self.recommendations_cache[recommendation.recommendation_id] = (
                recommendation
            )
        except Exception as e:
