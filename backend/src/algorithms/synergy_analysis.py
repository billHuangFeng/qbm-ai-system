...
class SynergyAnalysis:
    ...
    # 新增核心方法
    def calculate_shapley_values(self, X: pd.DataFrame, y: pd.Series, 
        ...
        try:
            ...
            for feature in feature_names:
                ...
                for subset_size in range(len(other_features) + 1):
                    for subset in itertools.combinations(other_features, subset_size):
                        ...
                        X_without = X[features_without] if features_without else X[[feature]].iloc[:, :0]
                        
                        # 训练模型并计算性能
                        if len(features_with) > 0:
                            model_with = RandomForestRegressor(n_estimators=50, random_state=42)
                        ...
                        if len(features_without) > 0:
                            model_without = RandomForestRegressor(n_estimators=50, random_state=42)
            ...
            return {
                ...
                'feature_ranking': sorted(shapley_values.items(), key=lambda x: x[1], reverse=True)
            }
    ...
    def _calculate_synergy_from_shapley(self, shapley_values: Dict[str, float], 
        ...
        try:
            ...
            for i, feature1 in enumerate(feature_names):
                for j, feature2 in enumerate(feature_names[i+1:], i+1):
                    ...
                    # 训练包含组合特征的模型
                    model_combined = RandomForestRegressor(n_estimators=50, random_state=42)
                    model_combined.fit(X_combined, y)
                    
                    # 计算组合特征的贡献
                    feature_importance = dict(zip(X_combined.columns, model_combined.feature_importances_))
                    combined_importance = feature_importance.get(f"{feature1}_x_{feature2}", 0)
    ...
    def analyze_nonlinear_synergy(self, X: pd.DataFrame, y: pd.Series, 
    ...
    def detect_threshold_synergy(self, X: pd.DataFrame, y: pd.Series, 
                               threshold_method: str = 'quantile') -> Dict[str, Any]:
        """
        检测阈值协同效应
        ...
        try:
            ...
            for feature in feature_names:
                ...
                # 创建阈值特征
                X_threshold = X.copy()
                X_threshold[f"{feature}_above_threshold"] = (X[feature] > threshold).astype(int)
                ...
                if above_threshold.sum() > 0 and below_threshold.sum() > 0:
                    ...
                    for other_feature in feature_names:
                        if other_feature != feature:
                            # 计算阈值特征与其他特征的交互
                            interaction_term = X_threshold[f"{feature}_above_threshold"] * X[other_feature]
    ...
    def _generate_insights(self) -> List[str]:
        ...
        try:
            ...
            if 'polynomial' in self.synergy_effects and self.synergy_effects['polynomial']:
                poly_improvement = self.synergy_effects['polynomial'].get('improvement', 0)
