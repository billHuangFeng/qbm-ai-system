...
class DataQualityAssessor(BaseService):
    ...
    def assess_accuracy(self, df: pd.DataFrame, field_configs: Dict[str, Dict[str, Any]]) -> Tuple[float, Dict[str, Any]]:
        """
        评估准确性（数据类型正确性、格式合规性）
        ...
        details = {
            "field_scores": {col: float(score) for col, score in zip(df.columns, accuracy_scores)},
            "issues": issues,
    ...
    def assess_consistency(self, df: pd.DataFrame, calculation_rules: List[Dict[str, Any]]) -> Tuple[float, Dict[str, Any]]:
        """
        评估一致性（计算字段一致性）
    ...
    def assess_timeliness(self, df: pd.DataFrame, date_fields: List[str]) -> Tuple[float, Dict[str, Any]]:
        """
        评估及时性（日期字段合理性）
        ...
        for field in date_fields:
            ...
            if field_issues:
                issues.append({
                    "field": field,
                    "issues": field_issues[:5]  # 只显示前5个问题
                })
        ...
        details = {
            ...
            "field_scores": {field: float(score) for field, score in zip(date_fields, timeliness_scores)},
            "issues": issues
        }
        
        return float(overall_score), details
    
    def assess_uniqueness(self, df: pd.DataFrame, primary_keys: List[str]) -> Tuple[float, Dict[str, Any]]:
        """
        评估唯一性（主键重复检查）
        ...
        for key_field in primary_keys:
            ...
            if duplicate_count > 0:
                # 找出重复的值
                duplicates = df[df[key_field].duplicated(keep=False)][key_field].unique()
                issues.append({
                    "field": key_field,
        ...
        details = {
            ...
            "field_scores": {key: float(score) for key, score in zip(primary_keys, uniqueness_scores)},
            "issues": issues
        }
        
        return float(overall_score), details
    
    def assess_validity(self, df: pd.DataFrame, validation_rules: Dict[str, Any]) -> Tuple[float, Dict[str, Any]]:
        """
        评估合规性（业务规则校验）
        ...
        score = (total_count - sum(i.get("count", 0) for i in issues)) / total_count if total_count > 0 else 1.0
        score = max(0.0, min(1.0, score))
        
        details = {
            "checked_rules": list(validation_rules.keys()),
            "issues": issues
        }
        ...
        for fk_config in foreign_keys:
            ...
            try:
                ...
                if total_count > 0:
                    ...
                    if invalid_count > 0:
                        invalid_values = [val for val in fk_values if val not in reference_set][:10]
                        issues.append({
                            "field": fk_field,
    ...
    def determine_importability(self, overall_score: float, blocking_issues: List[Dict[str, Any]]) -> str:
        """
        判定可导入性
    ...
    async def assess_quality(
        ...
        try:
            ...
            consistency_score, consistency_details = self.assess_consistency(df, calculation_rules)
            timeliness_score, timeliness_details = self.assess_timeliness(df, date_fields)
            uniqueness_score, uniqueness_details = self.assess_uniqueness(df, primary_keys)
            validity_score, validity_details = self.assess_validity(df, business_rules)
            
            # 关联性检查需要数据库访问
            if tenant_id:
                referential_score, referential_details = await self.assess_referential_integrity(
                    df, foreign_keys, tenant_id
                )
            else:
