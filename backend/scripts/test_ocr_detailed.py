...
async def test_ocr_directly():
    ...
    try:
        ...
        # 绘制多个文本行
        texts = ["Test OCR Recognition", "测试OCR文字识别", "2025-01-31"]
        ...
        if result.get("words"):
            ...
            for word in result.get("words", [])[:3]:
                print(
                    f"     - '{word.get('text')}' (置信度: {word.get('confidence')}%)"
                )
...
async def test_api_upload_with_ocr():
    ...
    try:
        ...
        # 上传到API
        print("\n2. 上传到API...")
        files = {"file": ("test_ocr.png", open(temp_file.name, "rb"), "image/png")}
        ...
        if response.status_code in [200, 201]:
            ...
            if result.get("knowledge_id"):
                ...
                detail_response = requests.get(
                    f"http://localhost:8081/expert-knowledge/{knowledge_id}", timeout=10
                )
