...
TEST_PAGINATION_DATA = [
    {"id": i, "name": f"item_{i}", "value": i * 10} for i in range(1, 101)
]
...
class TestSecurityConfig:
    ...
    def test_security_settings_validation(self):
        ...
        valid_config = SecuritySettings(
            jwt_secret_key="a" * 32, cors_origins=["http://localhost:3000"]
        )
...
class TestSecureDatabaseService:
    ...
    async def test_health_check_success(self, mock_db_service):
        """测试健康检查成功"""
        mock_db_service.pool.acquire.return_value.__aenter__.return_value.fetchrow.return_value = {
            "1": 1
        }
    ...
    async def test_health_check_failure(self, mock_db_service):
        """测试健康检查失败"""
        mock_db_service.pool.acquire.return_value.__aenter__.return_value.fetchrow.side_effect = Exception(
            "Connection failed"
        )
    ...
    async def test_safe_select(self, mock_db_service):
        ...
        mock_db_service.pool.acquire.return_value.__aenter__.return_value.fetch.return_value = (
            mock_result
        )
...
class TestPerformanceOptimization:
    ...
    async def test_paginated_query(self, mock_perf_service):
        ...
        result = await mock_perf_service.paginated_query(
            table="test_table", pagination=pagination
        )
    ...
    async def test_batch_load_with_relations(self, mock_perf_service):
        ...
        result = await mock_perf_service.batch_load_with_relations(
            main_table="main_table", relation_tables=relation_tables
        )
