...
class WeightValidation:
    ...
    def _cross_validation_validation(
        self, X: pd.DataFrame, y: pd.Series, weights: Dict[str, float]
    ) -> Dict[str, Any]:
        """交叉验证验证"""
        try:
            cv_results = {}
            ...
            cv_r2_scores = cross_val_score(
                LinearRegression(), X_weighted, y, cv=5, scoring="r2"
            )
    ...
    def _holdout_validation(
        ...
        try:
            ...
            holdout_results["overfitting_ratio"] = (
        ...
        try:
            ...
            try:
                ...
                bp_stat, bp_p, _, _ = het_breuschpagan(
                    residuals, np.ones((len(residuals), 1))
                )
    ...
    def _test_weight_significance(
        self, X_weighted: pd.DataFrame, y: pd.Series, weights: Dict[str, float]
    ) -> Dict[str, Any]:
        """测试权重显著性"""
        try:
            weight_significance = {}
            ...
            try:
                ...
                # p值
                p_values = 2 * (
                    1 - stats.t.cdf(np.abs(t_stats), len(y) - len(X_weighted.columns))
                )
    ...
    def _test_model_assumptions(
        self, X_weighted: pd.DataFrame, y: pd.Series, residuals: pd.Series
    ) -> Dict[str, Any]:
        """测试模型假设"""
        try:
            model_assumptions = {}
            ...
            # 3. 同方差假设
            homoscedasticity_assumption = self._test_homoscedasticity(
                X_weighted, residuals
            )
    ...
    def _test_homoscedasticity(
        self, X_weighted: pd.DataFrame, residuals: pd.Series
    ) -> Dict[str, Any]:
        """测试同方差假设"""
        try:
            # 使用Breusch-Pagan检验
        ...
        try:
            ...
            for i in range(n_bootstrap):
                ...
                weights_array = np.array(
                    [weights.get(feature, 0) for feature in X.columns]
                )
                X_weighted = X_bootstrap * weights_array
            ...
            for feature in X.columns:
                ...
                weight_stability[feature] = {
                    ...
                    "cv_weight": np.std(feature_weights)
                    / (np.mean(feature_weights) + 1e-8),
                    "weight_range": [np.min(feature_weights), np.max(feature_weights)],
                    "original_weight": weights.get(feature, 0),
                    "stability_score": 1
                    - np.std(feature_weights) / (np.mean(feature_weights) + 1e-8),
                }
        ...
        try:
            ...
            if "cross_validation" in results and results["cross_validation"]:
                ...
                if "mean_cv_r2" in cv_results:
                    score += weights["cross_validation"] * max(
                        0, cv_results["mean_cv_r2"]
                    )
            ...
            if "bootstrap" in results and results["bootstrap"]:
                ...
                if "mean_bootstrap_r2" in bootstrap_results:
                    score += weights["bootstrap"] * max(
                        0, bootstrap_results["mean_bootstrap_r2"]
                    )
            ...
            if "holdout" in results and results["holdout"]:
                ...
                if (
                    "test_scores" in holdout_results
                    and "r2" in holdout_results["test_scores"]
                ):
                    score += weights["holdout"] * max(
                        0, holdout_results["test_scores"]["r2"]
                    )
    ...
    def _calculate_statistical_score(
        ...
        try:
            ...
            if "model_assumptions" in statistical_results:
                ...
                if (
                    "linearity" in model_assumptions
                    and model_assumptions["linearity"]["is_linear"]
                ):
                    score += 0.25

                if (
                    "independence" in model_assumptions
                    and model_assumptions["independence"]["is_independent"]
                ):
                    score += 0.25

                if (
                    "homoscedasticity" in model_assumptions
                    and model_assumptions["homoscedasticity"]["is_homoscedastic"]
                ):
                    score += 0.25
    ...
    def compare_weight_sets(
        self, X: pd.DataFrame, y: pd.Series, weight_sets: Dict[str, Dict[str, float]]
    ) -> Dict[str, Any]:
        """比较多组权重"""
        try:
            comparison_results = {}
    ...
    def _analyze_weight_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.validation_history:
                ...
                performance_analysis["weight_performance_correlation"] = (
                    weight_performance_correlation
                )
    ...
    def _analyze_validation_trends(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(self.validation_history) > 1:
                # 时间趋势
                timestamps = [
                    validation["timestamp"] for validation in self.validation_history
                ]
                scores = [
                    validation["overall_score"]
                    for validation in self.validation_history
        ...
        try:
            report = {
                "summary": {
                    ...
                    "total_validations": len(self.validation_history),
                    "current_validation_score": self.validation_results.get(
                        "overall_score", 0
                    ),
                },
