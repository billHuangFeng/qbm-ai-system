...
class FieldMappingRequest(BaseModel):
    ...
    document_type: Optional[str] = Field(None, description="单据类型（SO/SH/SI/PO/RC/PI）")
...
class MappingHistoryRequest(BaseModel):
    ...
    mapping_method: str = Field("manual", description="映射方法（manual/rule/similarity）")
    ...
    try:
        ...
        if document_type:
            filtered_tables = [
                ...
                if t.get("document_type") == document_type
            ]
        else:
...
async def save_mapping_history(
    ...
    try:
        ...
        result = await db_service.fetch_one(upsert_sql, {
            'tenant_id': tenant_id,
            'source_system': request.source_system,
            'target_table': request.target_table,
        ...
        return MappingHistoryResponse(
            mapping_id=mapping_id or "unknown",
            success=True,
            message="映射历史保存成功"
        )
