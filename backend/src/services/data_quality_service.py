...
class DataQualityChecker:
    ...
    async def _check_completeness(
        ...
        try:
            ...
            # 计算完整性分数
            completeness_score = (
                1 - (missing_cells / total_cells) if total_cells > 0 else 1.0
            )
        ...
        try:
            ...
            for column in df.columns:
                ...
                if pd.api.types.is_numeric_dtype(column_data):
                    # 数值类型：检查异常值
                    score, column_issues = await self._check_numeric_accuracy(
                        column_data, column
                    )
                    ...
                    # 日期类型：检查日期格式和合理性
                    score, column_issues = await self._check_datetime_accuracy(
                        column_data, column
                    )
                    accuracy_scores.append(score)
                    issues.extend(column_issues)
                    ...
                    # 字符串类型：检查格式和内容
                    score, column_issues = await self._check_string_accuracy(
                        column_data, column
                    )
                    accuracy_scores.append(score)
                    issues.extend(column_issues)
    ...
    async def _check_numeric_accuracy(
        self, data: pd.Series, column: str
    ) -> Tuple[float, List[QualityIssue]]:
        """检查数值类型准确性"""
        try:
            issues = []
    ...
    async def _check_datetime_accuracy(
        self, data: pd.Series, column: str
    ) -> Tuple[float, List[QualityIssue]]:
        """检查日期类型准确性"""
        try:
    ...
    async def _check_string_accuracy(
        self, data: pd.Series, column: str
    ) -> Tuple[float, List[QualityIssue]]:
        """检查字符串类型准确性"""
        try:
            ...
            # 检查特殊字符
            special_chars = data[
                data.str.contains(r"[^\w\s\u4e00-\u9fff]", regex=True, na=False)
            ]
            ...
            for i, col1 in enumerate(df.columns):
                for j, col2 in enumerate(df.columns):
                    ...
                    if pd.api.types.is_numeric_dtype(
                        ...
                        score, column_issues = await self._check_numeric_consistency(
                    ...
                    # 检查分类字段的一致性
                        ...
                        df[col1]
                    ) and pd.api.types.is_string_dtype(df[col2]):
                        score, column_issues = (
                            await self._check_categorical_consistency(
                                df[col1], df[col2], col1, col2
                            )
                        )
                        consistency_scores.append(score)
    ...
    async def _check_numeric_consistency(
        self, col1: pd.Series, col2: pd.Series, col1_name: str, col2_name: str
    ) -> Tuple[float, List[QualityIssue]]:
        """检查数值字段一致性"""
        try:
            issues = []
            ...
            if pd.notna(correlation) and abs(correlation) > 0.8:
                # 高相关字段，检查逻辑一致性
                if col1_name.lower() in ["price", "amount"] and col2_name.lower() in [
                    "quantity",
                    "count",
                ]:
                    # 价格和数量应该成正比
    ...
    async def _check_categorical_consistency(
        ...
        try:
            ...
            # 检查分类组合的合理性
            if col1_name.lower() in ["status", "state"] and col2_name.lower() in [
                "category",
                "type",
            ]:
                # 状态和类别的组合检查
        ...
        try:
            ...
            for column in df.columns:
                ...
                for rule in validation_rules:
                    invalid_records = await self._apply_validation_rule(
                        column_data, rule
                    )
                ...
                # 计算有效性分数
                validity_score = (
                    1 - (invalid_count / len(column_data))
                    if len(column_data) > 0
                    else 1.0
                )
    ...
    async def _apply_validation_rule(
        self, data: pd.Series, rule: Dict[str, Any]
    ) -> List[int]:
        """应用验证规则"""
        try:
            invalid_indices = []
            ...
            if (
                ...
                for idx, value in data.items():
                    if pd.notna(value) and str(value).lower() not in [
    ...
    async def _check_uniqueness(
        ...
        try:
            ...
            for column in primary_key_candidates:
                ...
                total_count = len(df)
                uniqueness_ratio = (
                    unique_count / total_count if total_count > 0 else 1.0
                )
            ...
            # 计算总体唯一性分数
            overall_uniqueness = (
                np.mean(uniqueness_scores) if uniqueness_scores else 1.0
            )
        ...
        try:
            ...
            for column in time_columns:
                try:
                    ...
                    logger.error(
                        f"Timeliness check failed for column {column}: {str(e)}"
                    )
                    timeliness_scores.append(0.0)

            # 计算总体及时性分数
            overall_timeliness = (
                np.mean(timeliness_scores) if timeliness_scores else 1.0
            )
    ...
    async def _check_relevance(
        ...
        try:
            ...
            if len(numeric_columns) > 1:
                ...
                # 检查是否有冗余字段
                if (
                    len(high_correlations) > len(numeric_columns) * 0.3
                ):  # 高相关性字段对过多
                    issue = QualityIssue(
            ...
            for column in numeric_columns:
                ...
                if len(column_data) > 1:
                    cv = (
                        column_data.std() / column_data.mean()
                        if column_data.mean() != 0
                        else 0
                    )
    ...
    async def _apply_custom_rules(
        self, df: pd.DataFrame, custom_rules: List[QualityRule]
    ) -> List[QualityIssue]:
        """应用自定义规则"""
        try:
            issues = []
    ...
    async def _apply_threshold_rule(
    ...
    async def _apply_pattern_rule(
        ...
        try:
            ...
            # 应用正则表达式模式
            violations = field_data[
                ~field_data.astype(str).str.match(pattern, na=False)
            ]
    ...
    async def _apply_statistical_rule(
        ...
        try:
            ...
            z_scores = (
                np.abs((field_data - mean_val) / std_val)
                if std_val > 0
                else np.zeros(len(field_data))
            )
    ...
    async def _generate_recommendations(
        self, metrics_scores: Dict[QualityMetric, float], issues: List[QualityIssue]
    ) -> List[str]:
        """生成改进建议"""
        try:
            recommendations = []
            ...
            high_issues = [
                issue for issue in issues if issue.severity == IssueSeverity.HIGH
            ]
            ...
            for issue_type, count in issue_types.items():
                if count > 5:  # 某种类型的问题过多
                    recommendations.append(
                        f"重点关注 {issue_type} 相关问题，共 {count} 个"
                    )
