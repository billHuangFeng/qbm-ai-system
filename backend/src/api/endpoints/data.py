...
async def get_historical_data_by_id(
    ...
    if not historical_data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="历史数据不存在"
        )
...
async def get_data_quality_report(
    ...
    quality_stats = (
        db.query(HistoricalData)
        .filter(HistoricalData.tenant_id == current_user.tenant_id)
        .all()
    )

    total_records = len(quality_stats)
    if total_records == 0:
        return {"total_records": 0, "quality_score": 0, "quality_distribution": {}}
    ...
    for data in quality_stats:
        ...
        if quality_score >= 0.9:
            quality_distribution["excellent"] = (
                quality_distribution.get("excellent", 0) + 1
            )
