...
class RetrospectiveSessionCreate(BaseModel):
    ...
    session_type: str = Field(
        ..., description="会话类型: decision, project, quarterly, annual"
    )
...
class AnomalyDetectionRequest(BaseModel):
    ...
    anomaly_type: str = Field(
        "threshold", description="异常检测类型: threshold, statistical, pattern"
    )
...
class RootCauseAnalysisRequest(BaseModel):
    ...
    analysis_depth: str = Field(
        "comprehensive", description="分析深度: basic, comprehensive, deep"
    )
...
class PatternIdentificationRequest(BaseModel):
    ...
    pattern_type: str = Field(
        "all", description="模式类型: temporal, correlation, sequential, all"
    )
...
async def get_data_collector(
    ...
    return AIRetrospectiveDataCollector(
        db_service=db_service, memory_service=memory_service
    )
...
async def get_retrospective_analyzer(
    ...
    """获取复盘分析服务实例"""
    return AIRetrospectiveAnalyzer(db_service=db_service, memory_service=memory_service)
...
async def collect_feedback(
    ...
    try:
        ...
        result = await collector.collect_user_feedback(
            session_id=request.session_id, feedback_data=feedback_data
        )
...
async def extract_success_factors(
    ...
    try:
        result = await analyzer.extract_success_factors(
            success_cases=request.success_cases, session_id=request.session_id
        )
...
async def analyze_failure_reasons(
    ...
    try:
        result = await analyzer.analyze_failure_reasons(
            failure_cases=request.failure_cases, session_id=request.session_id
        )
...
async def suggest_process_optimizations(
    ...
    try:
        result = await recommender.suggest_process_optimizations(
            current_process=request.current_process, session_id=request.session_id
        )
...
async def create_risk_alerts(
    ...
    try:
        result = await recommender.create_risk_alerts(
            risk_indicators=request.risk_indicators, session_id=request.session_id
        )
