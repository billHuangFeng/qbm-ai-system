...
class WeightMonitoring:
    ...
    def monitor_weights(
        ...
        try:
            ...
            # 2. 性能监控
            performance_results = self._monitor_performance(
                X, y, weights, monitoring_config
            )
    ...
    def _detect_weight_drift(
        self, current_weights: Dict[str, float], config: Dict[str, Any]
    ) -> Dict[str, Any]:
        """检测权重漂移"""
        try:
            drift_results = {}
            ...
            for record in self.monitoring_history[
                -config.get("stability_window", 10) :
            ]:
            ...
            for feature in current_weights.keys():
                ...
                historical_weights_feature = [
                    hw.get(feature, 0) for hw in historical_weights
                ]
            ...
            if drift_scores:
                max_drift_feature = max(
                    drift_scores.keys(), key=lambda k: drift_scores[k]
                )
    ...
    def _monitor_performance(
        ...
        try:
            ...
            weights_array = np.array(
                [current_weights.get(feature, 0) for feature in X.columns]
            )
            ...
            if self.monitoring_history:
                ...
                for record in self.monitoring_history[
                    ...
                    if "performance_monitoring" in record["results"]:
                        perf = record["results"]["performance_monitoring"][
                            "current_performance"
                        ]
                ...
                if historical_performance:
                    ...
                    performance_results["performance_change"] = {
                        ...
                        "performance_degraded": mse_change
                        > config.get("performance_threshold", 0.05),
                    }

                    # 性能趋势
                    if len(historical_performance) > 1:
                        mse_trend = np.polyfit(
                            range(len(historical_mse)), historical_mse, 1
                        )[0]
                        r2_trend = np.polyfit(
                            range(len(historical_r2)), historical_r2, 1
                        )[0]
                        ...
                        performance_results["performance_trend"] = {
                            ...
                            "r2_trend": r2_trend,
                            "trend_direction": (
                                "improving" if r2_trend > 0 else "declining"
                            ),
                        }
    ...
    def _monitor_stability(
        ...
        try:
            ...
            # 计算平均稳定性
            avg_stability = (
                overall_stability / len(current_weights) if current_weights else 0
            )
        ...
        try:
            ...
            for feature in current_weights.keys():
                ...
                if len(feature_weights) > 2:
                    ...
                    is_anomaly = (
                        current_weight < lower_bound or current_weight > upper_bound
                    )
                    ...
                    if is_anomaly:
                        anomalies[feature] = {
                            ...
                            "severity": (
                                "high"
                                if abs(current_weight - np.mean(feature_weights))
                                > 2 * np.std(feature_weights)
                                else "medium"
                            ),
                        }
                        total_anomalies += 1
            ...
            if len(historical_weights) > 5:
                distribution_anomaly = self._detect_distribution_anomaly(
    ...
    def _detect_distribution_anomaly(
        self,
        current_weights: Dict[str, float],
        historical_weights: List[Dict[str, float]],
    ) -> Dict[str, Any]:
        """检测分布异常"""
        try:
            # 计算当前权重的分布特征
            ...
            is_distribution_anomaly = (
                mean_change > 2
                or std_change > 2
                or skewness_change > 1
                or kurtosis_change > 1
            )
    ...
    def _generate_alerts(
        self, monitoring_results: Dict[str, Any], config: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """生成警报"""
        try:
            alerts = []
                    ...
                    alerts.append(
                        {
                            "type": "weight_drift",
                            "severity": (
                                "high" if drift_data["drift_score"] > 0.2 else "medium"
                            ),
            ...
            if self.monitoring_history:
                ...
                for record in self.monitoring_history:
                    if "weight_drift" in record["results"]:
                        drift_score = record["results"]["weight_drift"].get(
                            "drift_score", 0
                        )
                        drift_scores.append(drift_score)

                    if "stability_monitoring" in record["results"]:
                        stability = record["results"]["stability_monitoring"].get(
                            "overall_stability", 0
                        )
            ...
            if self.monitoring_history:
                ...
                if len(performance_scores) > 1:
                    performance_trend = np.polyfit(
                        range(len(performance_scores)), performance_scores, 1
                    )[0]
                    performance_analysis["performance_trend"] = {
                        "slope": performance_trend,
                        "direction": (
                            "improving" if performance_trend > 0 else "declining"
                        ),
                ...
                if performance_scores:
                    performance_analysis["performance_stability"] = {
                        ...
                        "cv_r2": np.std(performance_scores)
                        / (np.mean(performance_scores) + 1e-8),
    ...
    def _analyze_alerts(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.monitoring_history:
                ...
                alert_analysis["alert_frequency"] = (
                    total_alerts / total_sessions if total_sessions > 0 else 0
                )
        ...
        try:
            report = {
                "summary": {
                    ...
                    "total_monitoring_sessions": len(self.monitoring_history),
                    "current_monitoring_status": self.monitoring_data.get(
                        "weight_drift", {}
                    ).get("status", "unknown"),
                },
