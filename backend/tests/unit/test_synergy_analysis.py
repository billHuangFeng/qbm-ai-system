...
class TestSynergyAnalysis:
    ...
    def test_analyze_pairwise_interactions(self):
        """测试两两交互效应分析"""
        result = self.synergy_analysis._analyze_pairwise_interactions(
            self.X, self.y, 0.1
        )
    ...
    def test_analyze_polynomial_interactions(self):
        """测试多项式交互效应分析"""
        result = self.synergy_analysis._analyze_polynomial_interactions(
            self.X, self.y, 0.1
        )
    ...
    def test_get_synergy_insights(self):
        ...
        # 检查协同效应水平
        assert insights["synergy_level"] in [
            "高协同效应",
            "中等协同效应",
            "低协同效应",
            "无显著协同效应",
    ...
    def test_analyze_feature_combinations(self):
        """测试特征组合分析"""
        result = self.synergy_analysis._analyze_feature_combinations(
            self.X, self.y, Mock()
        )
