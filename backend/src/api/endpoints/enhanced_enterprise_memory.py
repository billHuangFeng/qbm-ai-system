...
async def search_similar_patterns(
    query: str, limit: int = 5, current_user: User = Depends(get_current_user)
):
    """
    搜索相似模式
...
async def add_business_experience(
    ...
    try:
        ...
        experience = Experience(
            experience_id=enterprise_memory_service._generate_id(
                f"{scenario}_{datetime.now()}"
            ),
            scenario=scenario,
            context=context,
        ...
        return {"message": "业务经验添加成功", "experience": experience.dict()}
...
@router.get("/stats")
async def get_memory_statistics(current_user: User = Depends(get_current_user)):
    """
    获取企业记忆统计信息
...
async def demo_pattern_extraction(current_user: User = Depends(get_current_user)):
    """
    演示模式提取功能
    ...
    try:
        ...
        return {
            ...
            "recommendations": [
                rec.dict() for rec in recommendations[:3]
            ],  # 只返回前3个
        }
