...
class TestGetTableSchema:
    ...
    def test_get_table_schema_not_found(
        ...
        """测试表不存在"""
        response = client.get(
            "/api/v1/data-enhancement/table-schema/non_existent_table",
            headers=headers
        )
...
class TestGetAvailableTables:
    ...
    def test_get_available_tables_success(
        ...
        """测试获取可用表列表成功"""
        response = client.get(
            "/api/v1/data-enhancement/available-tables",
            headers=headers
        )
...
class TestFieldMappingIntegration:
    ...
    def test_complete_field_mapping_flow(
        ...
        # 验证历史映射的置信度更高
        order_no_rec = next(
            (r for r in recommendations2 if r["source_field"] == "订单号"),
            None
        )
        if order_no_rec:
            # 历史映射应该有更高的置信度
...
class TestPerformance:
    ...
    def test_get_table_schema_performance(
        ...
        start_time = time.time()
        response = client.get(
            "/api/v1/data-enhancement/table-schema/sales_order_header",
            headers=headers
        )
        end_time = time.time()
