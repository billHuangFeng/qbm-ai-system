...
from src.services.enhanced_model_training import (
...
async def test_model_training_service():
    ...
    X_class, y_class = model_training_service.prepare_data(
    ...
    X_reg, y_reg = model_training_service.prepare_data(
    ...
    # 重新训练模型
    model_class, _ = model_training_service.train_model(
        X_class_train, y_class_train, config_classification
    )
    model_reg, _ = model_training_service.train_model(
        X_reg_train, y_reg_train, config_regression
    )

    # 评估分类模型
    metrics_class = model_training_service.evaluate_model(
        model_class, X_class_test, y_class_test, config_classification
    )
    print(f"分类模型准确率: {metrics_class['accuracy']:.3f}")

    # 评估回归模型
    metrics_reg = model_training_service.evaluate_model(
        model_reg, X_reg_test, y_reg_test, config_regression
    )
    ...
    cv_scores_class = model_training_service.cross_validate_model(
        model_class, X_class_train, y_class_train
    )
    print(f"分类模型交叉验证分数: {cv_scores_class}")

    cv_scores_reg = model_training_service.cross_validate_model(
        model_reg, X_reg_train, y_reg_train
    )
