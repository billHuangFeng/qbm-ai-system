...
class AlgorithmService:
    ...
    def analyze_data_relationships(
        self, X: pd.DataFrame, y: pd.Series, analysis_types: List[str] = None
    ) -> Dict[str, Any]:
        """分析数据关系"""
        try:
            ...
            if "threshold" in analysis_types:
                threshold_results = self.threshold_analysis.detect_threshold_effects(
                    X, y
                )
            ...
            if "advanced" in analysis_types:
                advanced_results = (
                    self.advanced_relationships.analyze_complex_relationships(X, y)
                )
    ...
    def optimize_weights(
        ...
        try:
            ...
            # 3. 权重验证
            if (
                "ensemble" in optimized_weights
                and "mse" in optimized_weights["ensemble"]
            ):
    ...
    def predict_with_optimized_weights(
        self,
        X: pd.DataFrame,
        y: pd.Series,
        X_test: pd.DataFrame,
    ...
    def _create_enhanced_features(
        self, X: pd.DataFrame, analysis_results: Dict[str, Any]
    ) -> pd.DataFrame:
        """创建增强特征"""
        try:
            X_enhanced = X.copy()
            ...
            logger.info(
                f"增强特征创建完成，从 {len(X.columns)} 个特征增加到 {len(X_enhanced.columns)} 个特征"
            )
            return X_enhanced
    ...
    def _apply_weights(
        self, X: pd.DataFrame, weights: Dict[str, float]
    ) -> pd.DataFrame:
        """应用权重到特征"""
        try:
            X_weighted = X.copy()
    ...
    def _calculate_optimization_score(
        ...
        try:
            ...
            if "dynamic_weights" in optimization_results:
                dynamic_score = optimization_results["dynamic_weights"].get(
                    "overall_score", 0
                )
            ...
            if "optimized_weights" in optimization_results:
                ...
                if (
                    "ensemble" in optimized_results
                    and "mse" in optimized_results["ensemble"]
                ):
                    optimized_score = (
                        1 - optimized_results["ensemble"]["mse"]["ensemble_score"]
                    )  # 转换为正数
                else:
                    optimized_score = 0
            ...
            if "validation" in optimization_results:
                validation_score = optimization_results["validation"].get(
                    "overall_score", 0
                )
            ...
            if "monitoring" in optimization_results:
                monitoring_score = optimization_results["monitoring"].get(
                    "overall_score", 0
                )
