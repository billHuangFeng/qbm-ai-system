...
from ..error_handling.unified import (
    BMOSError,
    BusinessError,
    handle_errors,
...
class BaseService(ABC):
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cache_healthy = (
                await self._check_cache_health() if self.cache_service else True
            )
    ...
    @handle_errors
    async def get_by_id(
        self, table: str, id_value: Any, columns: Optional[List[str]] = None
    ...
    async def update_record(
        self, table: str, id_value: Any, data: Dict[str, Any]
    ) -> int:
        """更新记录"""
        data["updated_at"] = datetime.now()
        return await self.db_service.safe_update(
            table=table, data=data, where_clause="id = $1", where_params=[id_value]
        )
    ...
    async def delete_record(self, table: str, id_value: Any) -> int:
        ...
        return await self.db_service.safe_delete(
            table=table, where_clause="id = $1", where_params=[id_value]
        )
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                if attempt < self.config.max_retries - 1:
                    ...
                    self.logger.warning(
                        f"操作失败，重试 {attempt + 1}/{self.config.max_retries}: {e}"
                    )
                else:
...
class CRUDService(BaseService, Generic[T]):
    ...
    async def list_all(
        ...
        cache_key = self._generate_list_cache_key(
            where_clause, where_params, order_by, limit, offset
        )
