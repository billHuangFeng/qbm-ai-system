...
class AIDecisionRequirementsService:
        ...
        **kwargs
        ...
        try:
            ...
            priority_analysis = await self._analyze_priority(
                ...
                requirement_type, strategic_objective_id, required_by_date
            )
    ...
    async def _analyze_priority(
        ...
        try:
            ...
            feature_data = await self._prepare_priority_features(
                ...
                strategic_objective_id, required_by_date
            )
            ...
            # 3. 回退到基于规则的优先级分析
            return await self._rule_based_priority_analysis(feature_data, requirement_type)
    ...
    async def _prepare_priority_features(
        ...
        try:
            ...
            urgent_count = sum(1 for keyword in urgent_keywords 
                             if keyword.lower() in requirement_description.lower() or 
                             keyword.lower() in requirement_title.lower())
    ...
    async def _get_historical_requirement_data(
        self
    ...
    async def _find_similar_requirements(
        ...
        try:
                    ...
                    if description_keywords:
                        ...
                        for req in similar_db_reqs:
                            similar_requirements.append({
                                ...
                                "description": req.get("requirement_description", "")[:200],
            ...
            # 按相似度排序
            similar_requirements.sort(key=lambda x: x.get("similarity", 0), reverse=True)
    ...
    async def _recommend_best_practices(
        ...
        try:
            ...
            # 1. 搜索专家知识（理论框架）
            try:
                from ...services.expert_knowledge import ExpertKnowledgeService, KnowledgeSearchService
                ...
                if hasattr(self, 'knowledge_integration_service'):
                    ...
                    for knowledge in expert_knowledge:
                        recommendations.append({
                            "practice": knowledge.get("summary", knowledge.get("title", "")),
                            "pattern_type": knowledge.get("knowledge_type", "best_practice"),
                            "confidence": 0.8 if knowledge.get("verification_status") == "verified" else 0.6,
            ...
            if not recommendations:
                ...
                default_practices = type_practices.get(requirement_type, type_practices["operational"])
    ...
    async def _assess_risks(
        ...
        try:
            ...
            if similar_requirements:
                rejected_count = sum(1 for req in similar_requirements 
            ...
            return {
                ...
                "risks": risks,
                "overall_risk_level": "low" if len(risks) == 0 else 
                                     "medium" if len(risks) <= 2 else "high"
            }
