...
class AIRetrospectiveRecommender:
        ...
        try:
            ...
            if "success_factors" in analysis_results:
                success_suggestions = await self._suggestions_from_success(
                    analysis_results["success_factors"],
                    session_id
                )
    ...
    async def recommend_best_practices(
        ...
        try:
            ...
            # 3. 评估实践相关性
            relevant_practices = self._evaluate_practice_relevance(
                practices,
                context
            )
    ...
    async def create_risk_alerts(
        ...
        """
        try:
            logger.info(f"开始创建风险预警: session_id={session_id}, risk_count={len(risk_indicators)}")
            ...
            for indicator in risk_indicators:
                ...
                if risk_level in ["high", "critical"]:
                    alert = {
                        ...
                        "alert_message": self._generate_alert_message(indicator, risk_level),
                        "recommended_actions": await self._recommend_risk_actions(indicator, risk_level)
                    }
                    alerts.append(alert)
            
            # 2. 按风险级别排序
            alerts.sort(key=lambda x: ["low", "medium", "high", "critical"].index(x["risk_level"]), reverse=True)
            ...
            for alert in alerts:
                rec_id = await self._save_recommendation(
                    ...
                    expected_impact="high" if alert["risk_level"] in ["high", "critical"] else "medium"
                )
    ...
    async def _get_memory_based_suggestions(
        ...
        try:
            if self.memory_service:
                ...
                patterns = await self.memory_service.search_similar_patterns(query, limit=5)
    ...
    def _evaluate_and_rank_suggestions(
        ...
        for suggestion in suggestions:
            ...
            suggestion["score"] = (priority_score * 0.4 + impact_score * 0.6) * relevance_bonus
        
        # 按得分排序
        suggestions.sort(key=lambda x: x.get("score", 0), reverse=True)
        return suggestions
    
    async def _get_memory_practices(self, context: Dict[str, Any]) -> List[Dict[str, Any]]:
        """从企业记忆获取最佳实践"""
        practices = []
    ...
    async def _identify_bottlenecks(self, process: Dict[str, Any]) -> List[Dict[str, Any]]:
        """识别流程瓶颈"""
        bottlenecks = []
