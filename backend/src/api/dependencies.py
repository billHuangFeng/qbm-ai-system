...
async def get_current_analyst_user(current_user: Dict[str, Any] = Depends(get_current_user)) -> Dict[str, Any]:
    """获取当前分析师用户"""
...
class MockCacheService:
    ...
    async def get(self, namespace: str, key: str):
        return None
    async def set(self, namespace: str, key: str, value, expire_seconds: int | None = None):
        return True
...
def get_model_training_service() -> ModelTrainingService:
    ...
    if _model_training_service is None:
        ...
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="模型训练服务未初始化")
...
async def log_request(request: Request, current_user: Dict[str, Any] = Depends(get_current_user)):
    """记录请求日志"""
    logger.info(
