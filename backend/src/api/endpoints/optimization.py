...
async def create_optimization(
    ...
    try:
        ...
        if not optimization_data.title.strip():
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="优化建议标题不能为空"
            )

        if not optimization_data.description.strip():
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="优化建议描述不能为空"
            )
...
async def get_optimization(
    ...
    try:
        ...
        if not row:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="优化建议不存在"
            )
