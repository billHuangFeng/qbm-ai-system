...
class MemoryExtractionRequest(BaseModel):
    ...
    historical_evaluations: Optional[List[Dict[str, Any]]] = Field(
        [], description="历史评价数据"
    )
...
async def apply_memory_to_prediction(
    ...
    try:
        ...
        adjusted_prediction = await memory_service.apply_memory_to_prediction(
            base_prediction=request.base_prediction, memories=memories
        )
        ...
        return {
            ...
            "adjustment_summary": _generate_adjustment_summary(
                request.base_prediction, adjusted_prediction
            ),
        }
...
async def deactivate_memory(
    ...
    try:
        ...
        return {"success": True, "memory_id": memory_id, "message": "记忆已停用"}
