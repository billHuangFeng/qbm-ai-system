...
class AlignmentCheckRequest(BaseModel):
    ...
    check_type: str = Field(
        "full_alignment",
        description="检查类型: full_alignment, resource_conflict, goal_consistency, circular_dependency",
    )
    related_decision_ids: Optional[List[str]] = Field(
        None, description="相关决策ID列表"
    )
...
class RequirementAnalysisRequest(BaseModel):
    ...
    analysis_type: str = Field(
        "full", description="分析类型: full, similarity, threshold, optimization"
    )
...
async def analyze_requirement_depth(
    ...
    try:
        result = await service.analyze_requirement_depth(
            requirement_id=request.requirement_id, analysis_type=request.analysis_type
        )
...
async def get_similar_requirements(
    ...
    try:
        ...
        # 分析相似需求
        result = await service.analyze_requirement_depth(
            requirement_id=requirement_id, analysis_type="similarity"
        )

        return {
            "requirement_id": requirement_id,
            "similar_requirements": result.get("analysis_results", {}).get(
