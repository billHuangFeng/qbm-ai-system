...
class BMOSPerformanceTester:
    ...
    async def test_concurrent_requests(
        ...
        """测试并发请求性能"""
        print(
            f"测试并发性能: {concurrent_users}个用户, 每个用户{requests_per_user}个请求"
        )
        ...
        for size in test_data_sizes:
            ...
            try:
                async with httpx.AsyncClient() as client:
                    ...
                    response = await client.post(
                        f"{self.base_url}/api/v1/data-import/upload", files=files
                    )
    ...
    async def run_comprehensive_performance_test(self):
        ...
        for endpoint, method in basic_endpoints:
            ...
            result = await self.test_concurrent_requests(
                endpoint, method, concurrent_users=5, requests_per_user=10
            )
        ...
        if "model_training" in self.results:
            ...
            for result in self.results["model_training"]:
                if result["success"]:
                    ...
                    print(
                        f"  {result['algorithm']} ({result['data_size']}样本): 失败 - {result['error']}"
                    )
