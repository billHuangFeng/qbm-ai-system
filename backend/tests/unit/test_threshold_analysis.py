...
class TestThresholdAnalysis:
    ...
    def test_analyze_piecewise_regression(self):
        """测试分段回归分析"""
        result = self.threshold_analysis._analyze_piecewise_regression(
            self.X, self.y, 10
        )
    ...
    def test_calculate_threshold_score(self):
        ...
        threshold_results = {
            "decision_tree": {
                "feature1": {"effectiveness": {"threshold_0.5": {"improvement": 0.1}}}
            },
            "piecewise_regression": {
                "feature1": {"performance": {"r2_improvement": 0.2}}
            },
    ...
    def test_get_threshold_insights(self):
        ...
        # 检查阈值效应水平
        assert insights["threshold_level"] in [
            "高阈值效应",
            "中等阈值效应",
            "低阈值效应",
            "无显著阈值效应",
    ...
    def test_classify_threshold_level(self):
        ...
        assert (
            self.threshold_analysis._classify_threshold_level(0.05) == "无显著阈值效应"
        )
    ...
    def test_extract_key_thresholds(self):
        ...
        self.threshold_analysis.thresholds = {
            "decision_tree": {
                "feature1": {"effectiveness": {"threshold_0.5": {"improvement": 0.15}}}
            },
            "piecewise_regression": {
                "feature1": {
            ...
            "random_forest": {"feature1": {"best_threshold": 0.5, "improvement": 0.1}},
        }
    ...
    def test_generate_threshold_recommendations(self):
        ...
        recommendations_high = (
            self.threshold_analysis._generate_threshold_recommendations()
        )
    ...
    def test_clear_threshold_data(self):
        ...
        clear_threshold_y = pd.Series(
            np.where(
                clear_threshold_X["feature1"] > 0,
                clear_threshold_X["feature1"] * 3,
                clear_threshold_X["feature1"],
            )
        )

        result = self.threshold_analysis.detect_threshold_effects(
            clear_threshold_X, clear_threshold_y
        )
    ...
    def test_performance_with_large_data(self):
        ...
        large_X = pd.DataFrame(
            {f"feature_{i}": np.random.normal(0, 1, 1000) for i in range(5)}
        )
