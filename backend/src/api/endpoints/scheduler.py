...
async def create_task(
    request: TaskCreateRequest, scheduler: TaskScheduler = Depends(get_task_scheduler)
):
    """
    创建任务
...
async def get_task(
    task_id: str, scheduler: TaskScheduler = Depends(get_task_scheduler)
):
    """
    获取任务详情
...
@router.get("/stats")
async def get_scheduler_stats(scheduler: TaskScheduler = Depends(get_task_scheduler)):
    """
    获取调度器统计信息
...
async def get_registered_functions(
    ...
    try:
        ...
        return {"success": True, "functions": functions, "total_count": len(functions)}

    except Exception as e:
        logger.error(f"Failed to get registered functions: {str(e)}")
        raise HTTPException(status_code=500, detail=f"获取任务函数列表失败: {str(e)}")


@router.post("/start")
async def start_scheduler(scheduler: TaskScheduler = Depends(get_task_scheduler)):
    """
    启动调度器
...
@router.post("/stop")
async def stop_scheduler(scheduler: TaskScheduler = Depends(get_task_scheduler)):
    """
    停止调度器
...
async def get_schedule_types():
    ...
    try:
        ...
        return {"success": True, "types": types, "total_count": len(types)}

    except Exception as e:
        logger.error(f"Failed to get schedule types: {str(e)}")
        raise HTTPException(status_code=500, detail=f"获取调度类型失败: {str(e)}")
...
async def get_task_priorities():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"获取任务优先级失败: {str(e)}")
