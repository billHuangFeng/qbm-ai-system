...
class AuthenticationService:
    ...
    async def register_user(
        ...
        try:
            ...
            if existing_email:
                return AuthResult(
                    success=False, error="邮箱已存在", status=AuthStatus.UNAUTHENTICATED
                )
    ...
    async def authenticate_user(
        ...
        try:
            ...
            if not user.is_active:
                return AuthResult(
                    success=False, error="用户账户已被禁用", status=AuthStatus.REVOKED
                )
    ...
    async def verify_token(self, token: str) -> AuthResult:
        ...
        try:
            ...
            if await self._is_token_blacklisted(token):
                return AuthResult(
                    success=False, error="令牌已被撤销", status=AuthStatus.REVOKED
                )
            ...
            if not payload:
                return AuthResult(
                    success=False, error="无效的令牌", status=AuthStatus.UNAUTHENTICATED
                )

            # 检查令牌是否过期
            if datetime.fromtimestamp(payload["exp"]) < datetime.now():
                return AuthResult(
                    success=False, error="令牌已过期", status=AuthStatus.EXPIRED
                )
            ...
            return AuthResult(success=True, user=user, status=AuthStatus.AUTHENTICATED)
    ...
    async def check_permission(
        ...
        try:
            ...
            if not has_permission and permission == Permission.DATA_READ:
                has_permission = await self._check_cross_tenant_access(
                    user, resource_id
                )
    ...
    async def update_user_role(
        self, user_id: str, new_role: UserRole, updated_by: str
    ) -> bool:
        """更新用户角色"""
        try:
            # 检查更新者权限
        ...
        try:
            ...
            if result:
                ...
                await self.cache_service.set(
                    f"user:{user_id}", asdict(user), expire=3600  # 1小时
                )
        ...
        try:
            ...
            if payload:
                ...
                if expire_time > current_time:
                    ...
                    await self.cache_service.set(f"blacklist:{token}", True, expire=ttl)
    ...
    def _can_create_user(
        self, creator: User, target_role: UserRole, tenant_id: str
    ) -> bool:
        """检查是否可以创建用户"""
        # 超级管理员可以创建任何用户
    ...
    async def _check_cross_tenant_access(
        self, user: User, resource_id: Optional[str]
    ) -> bool:
        """检查跨租户访问权限"""
        try:
            # 检查用户是否有跨租户访问权限
            ...
            result = await self.db_service.execute_query(
                query, {"user_id": user.id, "resource_id": resource_id}
            )
...
class AuthorizationService:
    ...
    async def authorize_request(
        ...
        try:
            ...
            if tenant_id and user.tenant_id != tenant_id:
                # 检查跨租户访问权限
                if not await self.auth_service._check_cross_tenant_access(
                    user, resource_id
                ):
                    return False
            ...
            permission_check = await self.auth_service.check_permission(
                user.id, required_permission, resource_id
            )
