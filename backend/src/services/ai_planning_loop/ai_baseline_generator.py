...
class AIBaselineGenerator:
    ...
    async def generate_baseline(
        ...
        try:
            logger.info(
                f"开始生成基线: decision_id={decision_id}, baseline_name={baseline_name}"
            )
            ...
            if include_predictions:
                try:
                    ...
                    ai_predictions = await self._generate_default_predictions(
                        baseline_data, prediction_periods
                    )
            ...
            # 6. 风险评估
            risk_factors = await self._assess_baseline_risks(
                baseline_data, ai_predictions
            )
            ...
            try:
                ...
                if ts_data.shape[1] >= 3 and len(ts_data) >= 10:
                    ...
                    return {
                        ...
                        "model_performance": (
                            self.var_model.fitted_model.score
                            if hasattr(self.var_model, "fitted_model")
                            else None
                        ),
                    }
                else:
                    # 使用LightGBM进行单变量预测
                    return await self._predict_with_lightgbm(
                        ts_data, decision_data, prediction_periods
                    )
    ...
    def _prepare_time_series_data(
        self, historical_data: List[Dict[str, Any]], decision_data: Dict[str, Any]
            ...
            for record in historical_data[-20:]:  # 使用最近20条记录
                ...
                if isinstance(record, dict):
                    for key in ["revenue", "cost", "profit", "kpi_value"]:
                        if key in record:
                            record_dict[key] = (
                                float(record[key]) if record[key] is not None else 0.0
                            )
    ...
    def _get_last_features(
        self, series: pd.DataFrame, lags: int = 3
    ) -> Optional[List[float]]:
        """获取最后lags个值作为特征"""
        try:
            data = series.values.flatten()
    ...
    async def _optimize_baseline_parameters(
        self, baseline_data: Dict[str, Any], historical_data: List[Dict[str, Any]]
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    from ...services.enterprise_memory_service import (
                    ...
                    if self.db_service:
                        ...
                        expert_knowledge_methods = [
                            ...
                            in ["methodology", "theory", "best_practice"]
                        ]
                except Exception as e:
                ...
                for feature, importance in sorted(
                    ...
                    current_value = baseline_data.get("baseline_data", {}).get(
                        feature, 0
                    )
                    ...
                    if expert_knowledge_methods:
                        suggestion["expert_knowledge_reference"] = (
                            expert_knowledge_methods[0].get("summary", "")
                        )
                        suggestion["methodology"] = expert_knowledge_methods[0].get(
                            "title", ""
                        )
    ...
    async def _generate_default_predictions(
        self, baseline_data: Dict[str, Any], prediction_periods: int
    ) -> Dict[str, Any]:
        """生成默认预测（当AI模型不可用时）"""
        ...
        for kpi in target_kpis:
            ...
            target_value = (
                kpi.get("target_value", 100) if isinstance(kpi, dict) else 100
            )
    ...
    async def _assess_baseline_risks(
        self, baseline_data: Dict[str, Any], ai_predictions: Optional[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """评估基线风险"""
        risks = []
    ...
    async def _get_historical_baseline_data(
