...
class EnterpriseMemoryService:
        ...
        try:
            ...
            if 'metricAdjustments' in evaluation_data:
                patterns = self._extract_adjustment_patterns(
                    evaluation_data['metricAdjustments'],
            ...
            if 'evaluationContent' in evaluation_data:
                lessons = self._extract_lessons_learned(
                    evaluation_data['evaluationContent'],
                    historical_evaluations
                )
            ...
            if 'implementationPlan' in evaluation_data:
                strategies = self._extract_strategies(
                    evaluation_data['implementationPlan'],
                    historical_evaluations
                )
    ...
    def _extract_adjustment_patterns(
        ...
        if adjustment_reasons:
            ...
            patterns.append({
                ...
                'confidence_score': min(most_common_reason[1] / 3.0, 1.0),  # 3次以上为高置信度
        ...
        if relative_error > 0.5:  # 50%以上误差为严重错误
            memories.append({
                ...
                'memory_content': {
                    ...
                    'typical_causes': self._extract_common_causes(error_causes, historical_errors)
                },
    ...
    def retrieve_relevant_memories(
        ...
        relevant_memories.sort(
            key=lambda m: (m.get('relevance_score', 0) + m.get('confidence_score', 0)) / 2,
            reverse=True
        )
    ...
    def track_memory_effectiveness(
        ...
        try:
            ...
            supabase_client.table('memory_application_history').insert(application_history).execute()
            
            # 更新记忆统计
            self._update_memory_statistics(memory_id, application_result, supabase_client)
    ...
    def _update_memory_statistics(
        ...
        """更新记忆统计数据"""
        # 获取当前统计
        result = supabase_client.table('enterprise_memory').select('*').eq('id', memory_id).execute()
        ...
        new_success_rate = new_successful_count / new_applied_count if new_applied_count > 0 else 0
    ...
    def extract_memory_from_prediction_error(
        ...
        try:
            ...
            # 2. 提取异常模式
            anomaly_patterns = self._extract_anomaly_patterns(error_data, historical_errors)
            memories.extend(anomaly_patterns)
            
            # 3. 提取优化规则
            optimization_rules = self._extract_optimization_rules(error_data, historical_errors)
            ...
            for memory in memories:
                ...
                if adjustment_applied:
                    applied_memories.append({
                        ...
                        'adjustment_factor': memory_content.get('adjustment_factor', 1.0),
    ...
    def track_memory_effectiveness(
        ...
        try:
            ...
            application_history = {
                'memory_id': memory_id,
                'application_context': application_result.get('context_id'),
                'application_type': application_result.get('type', 'prediction_adjustment'),
                'was_successful': application_result.get('success', False),
                'impact_score': application_result.get('impact', 0),
                'actual_result': application_result.get('actual'),
                'expected_result': application_result.get('expected'),
    ...
    def _analyze_error_patterns(
        ...
        # 统计相似误差
        similar_errors = [
            err for err in historical_errors 
        ...
        if len(similar_errors) >= 2:  # 至少3次相似误差才形成模式
            patterns.append({
                ...
                'memory_content': {
                    ...
                    'common_causes': self._extract_common_error_causes(similar_errors)
                },
        ...
        if relative_error > 0.5:  # 50%以上误差
            patterns.append({
                ...
                'memory_content': {
                    'severity_level': 'high',
                    'error_threshold': 0.5,
                    'typical_causes': self._extract_common_error_causes(historical_errors),
    ...
    def _extract_optimization_rules(
        ...
        if len(historical_errors) >= 3:
            avg_error = np.mean([err.get('relative_error', 0) for err in historical_errors])
