...
class IntelligentFieldMapper(BaseService):
    ...
    async def recommend_mappings(
        ...
        if not target_table:
            raise ValueError(
                "target_table 是必需的，必须提供目标表名以从数据库获取字段"
            )
    ...
    async def _get_history_mappings(
        ...
        try:
            ...
            """
            params = {"source_field": source_field, "source_system": source_system}
    ...
    def _apply_mapping_rules(
        self, source_field: str, source_system: str, document_type: Optional[str]
    ) -> List[MappingCandidate]:
        """应用映射规则"""
        candidates = []
        ...
        for rule in rules:
            ...
            if self._match_pattern(
                source_field, rule["source_pattern"], rule["match_type"]
            ):
                candidates.append(
                    MappingCandidate(
    ...
    def _calculate_similarity_mappings(
        self, source_field: str, standard_fields: List[str]
    ) -> List[MappingCandidate]:
        """计算相似度匹配
    ...
    async def _get_standard_fields_from_db(
        ...
        try:
            ...
            table_fields, master_data_fields = await asyncio.gather(
                table_fields_task, master_data_fields_task, return_exceptions=True
            )
    ...
    async def _get_target_table_fields(
        self, target_table: str, use_cache: bool = True
    ) -> List[str]:
        """获取目标表的所有字段名（带缓存）
        ...
        try:
            ...
            results = await self.db_service.fetch_all(
                query_sql, {"table_name": target_table}
            )
    ...
    async def _get_master_data_match_fields(
        ...
        if use_cache and self.cache_service:
            try:
                cached_data = await self.cache_service.get(
                    "master_data_fields", target_table
                )
            ...
            if not fk_results:
                ...
                for id_field in id_fields:
                    if id_field in id_to_table_mapping:
                        ...
                        # 获取主数据表的匹配字段
                        master_fields = await self._get_master_table_match_fields(
                            master_table
                        )
    ...
    async def _get_foreign_keys(
        ...
        try:
            ...
            results = await self.db_service.fetch_all(
                fk_query, {"table_name": target_table}
            )
    ...
    async def _get_master_table_match_fields(
        self, master_table: str, use_cache: bool = True
    ) -> List[str]:
        """获取主数据表的匹配字段（带缓存）
        ...
        try:
            ...
            raise ValueError(
                f"无法获取主数据表 {master_table} 的匹配字段: {str(e)}"
            ) from e
    ...
    async def preload_table_cache(self, tables: List[str]):
        ...
        # 并发预加载所有表
        tasks = [
            self._get_standard_fields_from_db(table, use_cache=True) for table in tables
        ]
    ...
    def _deduplicate_and_sort_candidates(
        ...
        for candidate in candidates:
            ...
            if (
                ...
                or candidate.confidence > unique_candidates[key].confidence
            ):
                unique_candidates[key] = candidate

        # 按置信度排序
        return sorted(
            unique_candidates.values(), key=lambda x: x.confidence, reverse=True
        )
